...
export default function AIAssistant() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      type: 'assistant',
      content: "Hello! I'm Lucky, your AI assistant. I'm monitoring the platform 24/7 for fraud, unusual patterns, and system issues. How can I help you today?",
  ...
  const [alerts, setAlerts] = useState<Alert[]>([
    {
      ...
      description: 'User "casinofan" showing unusual betting patterns. 47 consecutive losses followed by large win.',
      priority: 'high',
    ...
    },
    {
      ...
      description: 'Sweet Bonanza RTP deviation: 102.3% over last 1000 spins (expected: 96.48%)',
      priority: 'medium',
    ...
    },
    {
      ...
      description: 'Database response time increased by 300%. Investigating potential cause.',
      priority: 'medium',
    ...
    },
    {
      ...
      description: 'Successfully resolved 12 password reset requests and 3 bonus claim inquiries automatically.',
      priority: 'low',
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setLiveMetrics(prev => ({
        ...prev,
        fraudScore: Math.max(0, Math.min(10, prev.fraudScore + (Math.random() - 0.5) * 0.5)),
        ...
        autoResolvedToday: prev.autoResolvedToday + Math.floor(Math.random() * 2)
      }));
    }, 5000);
    ...
    <div className="fixed bottom-6 right-6 w-96 h-[600px] bg-card border border-border rounded-lg shadow-xl z-50 flex flex-col">
      ...
      <div className="p-3 border-b border-border bg-muted/20">
        <div className="grid grid-cols-3 gap-2 text-xs">
          <div className="text-center">
            <div className={`font-bold ${liveMetrics.fraudScore > 5 ? 'text-red-500' : liveMetrics.fraudScore > 3 ? 'text-orange-500' : 'text-green-500'}`}>
              {liveMetrics.fraudScore.toFixed(1)}
            </div>
            <div className="text-muted-foreground">Fraud Score</div>
          </div>
          <div className="text-center">
            <div className={`font-bold ${liveMetrics.systemHealth < 95 ? 'text-red-500' : liveMetrics.systemHealth < 98 ? 'text-orange-500' : 'text-green-500'}`}>
              {liveMetrics.systemHealth.toFixed(1)}%
            </div>
            <div className="text-muted-foreground">System Health</div>
          </div>
          <div className="text-center">
            <div className="font-bold text-casino-blue">{liveMetrics.activeUsers}</div>
      ...
      <div className="p-3 border-b border-border max-h-32 overflow-y-auto">
        ...
        <div className="space-y-1">
          {alerts.filter(a => a.status === 'active').slice(0, 2).map(alert => (
            <div key={alert.id} className="flex items-center justify-between p-2 bg-red-500/10 border border-red-500/20 rounded text-xs">
              <div className="flex-1">
                <div className="font-medium text-red-400">{alert.title}</div>
                <div className="text-muted-foreground truncate">{alert.description}</div>
              </div>
              <div className="flex gap-1">
                <Button size="sm" variant="outline" onClick={() => investigateAlert(alert.id)} className="h-6 px-2 text-xs">
                  <Eye className="w-3 h-3" />
                </Button>
                <Button size="sm" variant="outline" onClick={() => resolveAlert(alert.id)} className="h-6 px-2 text-xs">
            ...
            <div className={`max-w-[80%] p-2 rounded-lg text-sm ${
              ...
              {message.type === 'assistant' && (
                <div className="flex items-center gap-1 mb-1">
                  ...
                  <span className="text-xs font-medium text-gold-500">Lucky</span>
                </div>
              )}
              {message.content}
      ...
      <div className="p-3 border-t border-border">
        <div className="flex gap-2">
          <Input
            placeholder={isListening ? "Listening..." : "Ask Lucky about fraud, RTP, or system status..."}
