...
interface PokerTable {
  ...
  stakes: { GC: { small: number; big: number }; SC: { small: number; big: number } };
  players: number;
  maxPlayers: number;
...
export default function BingoPokerGames() {
  ...
  const [activeBingoGame, setActiveBingoGame] = useState<BingoGame | null>(null);
  const [bingoCards, setBingoCards] = useState<BingoCard[]>([]);
  const [selectedBingoRoom, setSelectedBingoRoom] = useState<string | null>(null);
  ...
  const [activePokerGame, setActivePokerGame] = useState<PokerGame | null>(null);
  const [selectedPokerTable, setSelectedPokerTable] = useState<string | null>(null);
  ...
  const initializeBingo = async () => {
    const rooms: BingoRoom[] = [
      ...
      },
      {
        ...
        isVIP: true,
        features: ['Massive Jackpots', 'VIP Chat', 'Personal Host', 'Premium Patterns']
      }
    ];
    ...
    const tables: PokerTable[] = [
      {
        ...
        stakes: { GC: { small: 10, big: 20 }, SC: { small: 0.10, big: 0.20 } },
        players: 4,
        maxPlayers: 6,
        status: 'playing',
        isVIP: false,
      ...
      },
      {
        ...
        name: 'Low Stakes',
        gameType: 'texas_holdem',
        stakes: { GC: { small: 50, big: 100 }, SC: { small: 0.50, big: 1.0 } },
        players: 6,
        maxPlayers: 8,
        status: 'playing',
        isVIP: false,
  ...
  const startBingoCountdown = (game: BingoGame) => {
    ...
    const countdown = setInterval(() => {
      ...
      setActiveBingoGame(prev => prev ? { ...prev, timeToNext: timeLeft } : null);
  ...
  const startBingoGame = (game: BingoGame) => {
    setActiveBingoGame(prev => prev ? { ...prev, status: 'playing', timeToNext: 0 } : null);
    ...
    const drawNumber = () => {
      ...
      if (drawnCount < maxDraws && drawnCount < 25) { // End game early for demo
  ...
  const claimBingoWin = async (game: BingoGame) => {
    ...
    try {
      const prizeAmount = currency === 'GC' ? game.prizePool.GC : game.prizePool.SC;
  ...
  const endBingoGame = () => {
    ...
    setActiveBingoGame(prev => prev ? { ...prev, status: 'finished' } : null);
  ...
  const joinPokerTable = async (tableId: string) => {
    ...
    const bigBlind = currency === 'GC' ? table.stakes.GC.big : table.stakes.SC.big;
    ...
    try {
      ...
      const game: PokerGame = {
        ...
        communityCards: [],
        pot: { GC: 0, SC: 0 },
        currentBet: { GC: currency === 'GC' ? table.stakes.GC.big : 0, SC: currency === 'SC' ? table.stakes.SC.big : 0 },
        playerBet: { GC: 0, SC: 0 },
        phase: 'preflop',
        playerAction: null,
        opponentAction: '',
  ...
  const startPokerActionTimer = (game: PokerGame) => {
    ...
    const timer = setInterval(() => {
      ...
      setActivePokerGame(prev => prev ? { ...prev, timeToAct: timeLeft } : null);
  ...
  const handlePokerAction = async (action: 'fold' | 'call' | 'raise' | 'check' | 'all_in') => {
    ...
    switch (action) {
      ...
      case 'call':
        const callAmount = currency === 'GC' ? game.currentBet.GC : game.currentBet.SC;
        game.playerBet = currency === 'GC' ? { ...game.playerBet, GC: callAmount } : { ...game.playerBet, SC: callAmount };
        break;
        
      case 'raise':
        const raiseAmount = currency === 'GC' ? game.currentBet.GC * 2 : game.currentBet.SC * 2;
        game.playerBet = currency === 'GC' ? { ...game.playerBet, GC: raiseAmount } : { ...game.playerBet, SC: raiseAmount };
        game.currentBet = currency === 'GC' ? { ...game.currentBet, GC: raiseAmount } : { ...game.currentBet, SC: raiseAmount };
        break;
  ...
  const evaluatePokerHand = async (game: PokerGame) => {
    ...
    if (playerWins) {
      const potAmount = currency === 'GC' ? game.currentBet.GC * 2 : game.currentBet.SC * 2;
    ...
    return (
      <div className="bg-white rounded-lg p-4 shadow-lg border-2 border-blue-500">
        <div className="grid grid-cols-5 gap-1 mb-2">
          {headers.map(header => (
            <div key={header} className="bg-blue-600 text-white font-bold text-center py-2 rounded">
              {header}
            </div>
          ))}
        </div>
                      ...
                      setBingoCards(prev => 
                        prev.map(c => 
                          c.id === card.id 
                            ? {
                                ...c,
                                markedNumbers: c.markedNumbers.map((colMarked, cIndex) =>
                                  cIndex === col 
  ...
  const renderPokerCard = (card: PokerCard, hidden: boolean = false) => {
    ...
    return (
      <div className="w-12 h-16 bg-white rounded border-2 border-gray-300 flex flex-col items-center justify-between p-1 text-xs font-bold">
        ...
        {suitIcons[card.suit]}
        <div className={`${card.suit === 'hearts' || card.suit === 'diamonds' ? 'text-red-500' : 'text-black'} transform rotate-180`}>
          {card.rank}
        </div>
      </div>
    );
  ...
  return (
    <div className="container mx-auto px-4 py-6">
      ...
      <div className="mb-6">
        ...
        <p className="text-muted-foreground">Classic games with exciting prizes and jackpots</p>
          ...
          <div className="flex items-center gap-4 mt-4">
            <Card className="px-4 py-2">
              <div className="flex items-center gap-2">
                ...
                <span className="font-semibold">{wallet.goldCoins.toLocaleString()} GC</span>
              </div>
            </Card>
            <Card className="px-4 py-2">
              <div className="flex items-center gap-2">
      ...
      <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as 'bingo' | 'poker')} className="space-y-6">
                ...
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <CardTitle className="flex items-center gap-2">
                      ...
                      <Badge className={activeBingoGame.status === 'live' ? 'bg-green-600' : activeBingoGame.status === 'waiting' ? 'bg-yellow-600' : 'bg-gray-600'}>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      >
                        {bingoAudioEnabled ? <Volume2 className="w-4 h-4" /> : <VolumeX className="w-4 h-4" />}
                      </Button>
                      ...
                      <Button variant="destructive" size="sm" onClick={endBingoGame}>
                        Leave Game
                      </Button>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="space-y-4">
                      {activeBingoGame.status === 'waiting' && (
                        <Card>
                          <CardContent className="pt-6 text-center">
                            ...
                            <h3 className="font-semibold mb-2">Game Starting Soon</h3>
                      ...
                      {activeBingoGame.nextNumber && (
                        <Card>
                          <CardContent className="pt-6 text-center">
                            <h3 className="font-semibold mb-2">Latest Number</h3>
                        ...
                        <CardContent className="pt-6">
                          <div className="space-y-2 text-sm">
                            <div className="flex justify-between">
                              <span>Players:</span>
                              <span>{activeBingoGame.players}/{activeBingoGame.maxPlayers}</span>
                            </div>
                            <div className="flex justify-between">
                              <span>Prize Pool:</span>
                              <span className="font-semibold">
                                {currency === 'GC' ? activeBingoGame.prizePool.GC : activeBingoGame.prizePool.SC} {currency}
                              </span>
                            </div>
                            <div className="flex justify-between">
                              <span>Pattern:</span>
                              <span className="capitalize">{activeBingoGame.pattern.replace('_', ' ')}</span>
                            </div>
                          </div>
                        </CardContent>
                      </Card>
              ...
              {bingoRooms.map(room => (
                <Card key={room.id} className="hover:shadow-lg transition-shadow">
                  <CardHeader>
                    <div className="flex items-center justify-between">
                      <CardTitle className="flex items-center gap-2">
                        {room.isVIP && <Crown className="w-5 h-5 text-gold-500" />}
                        {room.name}
                      </CardTitle>
                      <Badge variant={room.players < room.maxPlayers ? "default" : "destructive"}>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div>
                        ...
                        <div>{room.cardCost.GC} GC / {room.cardCost.SC} SC</div>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <div>{Math.ceil((room.nextGame.getTime() - Date.now()) / 60000)}m</div>
                      </div>
                      <div>
                ...
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <CardTitle className="flex items-center gap-2">
                      ...
                      {pokerTables.find(t => t.id === selectedPokerTable)?.name}
                      <Badge>{activePokerGame.phase.toUpperCase()}</Badge>
                    </CardTitle>
                    
                    <div className="flex items-center gap-4">
                      <div className="text-sm">
                        Time to act: <span className="font-bold">{activePokerGame.timeToAct}s</span>
                      </div>
                      <Button variant="destructive" size="sm" onClick={() => endPokerHand(false)}>
                        Leave Table
                      </Button>
                    </div>
                  </div>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                      ...
                      <div className="text-center">
                        ...
                        {/* Action Buttons */}
                        {activePokerGame.isPlayerTurn && activePokerGame.phase !== 'showdown' && (
                          <div className="flex justify-center gap-2">
                            <Button variant="destructive" onClick={() => handlePokerAction('fold')}>
                              Fold
                            </Button>
                            <Button variant="outline" onClick={() => handlePokerAction('check')}>
                              Check
                            </Button>
                            <Button onClick={() => handlePokerAction('call')}>
                              Call {currency === 'GC' ? activePokerGame.currentBet.GC : activePokerGame.currentBet.SC} {currency}
                            </Button>
                            <Button variant="secondary" onClick={() => handlePokerAction('raise')}>
                              Raise
                            </Button>
                          </div>
                        )}
                      ...
                      <div className="space-y-4">
                          ...
                          <CardContent className="pt-6">
                            <div className="space-y-2 text-sm">
                              <div className="flex justify-between">
                                <span>Pot:</span>
                                <span className="font-semibold">
                                  {currency === 'GC' ? activePokerGame.pot.GC : activePokerGame.pot.SC} {currency}
                                </span>
                              </div>
                              <div className="flex justify-between">
                                <span>Current Bet:</span>
                                <span>
                                  {currency === 'GC' ? activePokerGame.currentBet.GC : activePokerGame.currentBet.SC} {currency}
                                </span>
                              </div>
                              <div className="flex justify-between">
                                <span>Your Bet:</span>
                                <span>
                                  {currency === 'GC' ? activePokerGame.playerBet.GC : activePokerGame.playerBet.SC} {currency}
                                </span>
                              </div>
                            </div>
                          </CardContent>
                        ...
                        {activePokerGame.opponentAction && (
                          <Card>
                            <CardContent className="pt-6 text-center">
                              <h4 className="font-semibold mb-2">Opponent Action</h4>
                              <Badge className="capitalize">{activePokerGame.opponentAction}</Badge>
                            </CardContent>
                          </Card>
                        )}
                      </div>
              ...
              {pokerTables.map(table => (
                <Card key={table.id} className="hover:shadow-lg transition-shadow">
                  <CardHeader>
                    <div className="flex items-center justify-between">
                      <CardTitle className="flex items-center gap-2">
                        {table.isVIP && <Crown className="w-5 h-5 text-gold-500" />}
                        {table.name}
                      </CardTitle>
                      <Badge variant={table.status === 'playing' ? "default" : table.status === 'waiting' ? "secondary" : "outline"}>
                        {table.status}
                      </Badge>
                    </div>
                  </CardHeader>
                  ...
                  <CardContent className="space-y-4">
                    <div className="text-sm space-y-2">
                      <div className="flex justify-between">
                        <span>Game:</span>
                        <span className="capitalize font-medium">{table.gameType.replace('_', ' ')}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Stakes:</span>
                        <span className="font-medium">
                          {currency === 'GC' ? `${table.stakes.GC.small}/${table.stakes.GC.big}` : `${table.stakes.SC.small}/${table.stakes.SC.big}`} {currency}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span>Players:</span>
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="text-green-600 font-semibold">
                          {currency === 'GC' ? table.avgPot.GC : table.avgPot.SC} {currency}
                        </span>
                      </div>
                      <div className="flex justify-between">
                    ...
                    >
                      <Play className="w-4 h-4 mr-2" />
                      {table.players >= table.maxPlayers ? 'Table Full' : 'Join Table'}
                    </Button>
                  </CardContent>
                </Card>
              ))}
