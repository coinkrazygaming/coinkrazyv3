...
export default function GameManagement() {
  ...
  const [selectedGame, setSelectedGame] = useState<
    SlotGameManagement | TableGameManagement | null
  >(null);
    ...
    try {
      if ("reels" in editingGame) {
        ...
        setSlotGames((games) =>
          games.map((game) =>
            game.id === editingGame.id
              ? (editingGame as SlotGameManagement)
              : game,
          ),
        );
      } else {
        ...
        setTableGames((games) =>
          games.map((game) =>
            game.id === editingGame.id
              ? (editingGame as TableGameManagement)
              : game,
          ),
        );
      }
      ...
      <CardHeader className="pb-3">
        <div className="flex justify-between items-start">
          <div>
            ...
            <CardDescription>
              {game.theme} • {game.reels}x{game.rows}
            </CardDescription>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
          <div>
            <div className="text-2xl font-bold">
              {game.stats.totalPlays.toLocaleString()}
            </div>
            <div className="text-sm text-muted-foreground">Total Plays</div>
          </div>
          <div>
            <div className="text-2xl font-bold">
              {formatCurrency(game.stats.totalWagered)}
            </div>
            <div className="text-sm text-muted-foreground">Total Wagered</div>
          </div>
          <div>
            <div className="text-2xl font-bold">
              {formatCurrency(game.stats.profit)}
            </div>
            <div className="text-sm text-muted-foreground">Profit</div>
          </div>
          <div>
            <div className="text-2xl font-bold">
              {game.stats.rtp.toFixed(1)}%
            </div>
      ...
      <CardHeader className="pb-3">
        <div className="flex justify-between items-start">
          <div>
            ...
            <CardDescription>
              {game.type} • House Edge: {(game.houseEdge * 100).toFixed(2)}%
            </CardDescription>
          </div>
          <div className="text-right">
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div>
            <div className="text-lg font-bold">
              {provider.supportedGames.length}
            </div>
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">
            Manage slot games, table games, and live game providers
          </p>
        </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="tables" className="space-y-4">
          <div className="flex justify-between items-center">
            <h3 className="text-xl font-semibold">
              Table Games ({tableGames.length})
            </h3>
            <Button
              onClick={() => {
                setNewGameType("table");
                setNewGameDialog(true);
              }}
            >
              <Plus size={16} className="mr-2" />
        ...
        <TabsContent value="live" className="space-y-4">
          <div className="flex justify-between items-center">
            <h3 className="text-xl font-semibold">
              Live Game Providers ({liveProviders.length})
            </h3>
            <Button>
              <Plus size={16} className="mr-2" />
              Add Provider
      ...
      <Dialog open={isEditing} onOpenChange={setIsEditing}>
        <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Modify game settings and configuration
            </DialogDescription>
          </DialogHeader>
          {editingGame && (
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                {"reels" in editingGame && (
                  <>
                    ...
                    <div>
                      <Label htmlFor="jackpot-contribution">
