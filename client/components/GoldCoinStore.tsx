...
const GOLD_COIN_PACKAGES: GoldCoinPackage[] = [
  {
    ...
    features: ['5,000 Gold Coins', '500 Bonus GC', '5 Sweeps Coins', 'Instant delivery']
  },
  {
    id: 'popular',
    name: 'Popular Choice',
    ...
    features: ['15,000 Gold Coins', '2,000 Bonus GC', '15 Sweeps Coins', '25% Extra Value', 'Priority support']
  },
  {
    id: 'value',
    name: 'Best Value',
    ...
    features: ['35,000 Gold Coins', '7,000 Bonus GC', '35 Sweeps Coins', '25% Extra Value', 'VIP Support', 'Exclusive bonuses']
  },
  {
    id: 'premium',
    name: 'Premium Pack',
    ...
    features: ['75,000 Gold Coins', '20,000 Bonus GC', '75 Sweeps Coins', '25% Extra Value', 'VIP Support', 'Monthly bonus', 'Exclusive games access']
  },
  {
    id: 'ultimate',
    name: 'Ultimate Pack',
    ...
    features: ['150,000 Gold Coins', '50,000 Bonus GC', '150 Sweeps Coins', '33% Extra Value', 'Dedicated VIP Manager', 'Weekly bonuses', 'All exclusive games', 'Priority withdrawals']
  }
];
...
const GoldCoinStore = () => {
  ...
  const loadPurchaseHistory = async () => {
    try {
      ...
      const transactions = await realNeonService.getUserTransactions(currentUser.id, 20);
      const purchases = transactions.filter(tx => tx.type === 'deposit' || tx.type === 'purchase');
  ...
  const calculateCustomPackage = (amount: number) => {
    ...
    if (amount >= 50) {
      ...
      bonusGC = Math.floor(goldCoins * 0.10); // 10% bonus for $10+
  ...
  const processPurchase = async () => {
    ...
    try {
      ...
      alert(`Purchase successful! You received ${totalGC.toLocaleString()} Gold Coins and ${selectedPackage.sweepsCoins} Sweeps Coins!`);
  ...
  const processCustomPurchase = async () => {
    ...
    try {
      ...
      const { goldCoins, bonusGC, sweepsCoins } = calculateCustomPackage(amount);
      ...
      alert(`Custom purchase successful! You received ${totalGC.toLocaleString()} Gold Coins and ${sweepsCoins} Sweeps Coins!`);
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Purchase Gold Coins and Sweeps Coins to enhance your gaming experience
          </CardDescription>
        </CardHeader>
      </Card>
        ...
        <Card className="bg-gradient-to-r from-purple-500/10 to-blue-500/10 border-purple-200">
          <CardContent className="pt-6">
            ...
            <p className="text-sm text-muted-foreground">
              You get an additional {(userVIP.level * 2)}% discount on all purchases as a Level {userVIP.level} VIP member!
            </p>
          </CardContent>
        </Card>
      )}
                ...
                >
                  ...
                  <CardHeader className="text-center">
                    ...
                    <div className="space-y-2">
                      <div className="text-3xl font-bold">
                        ${isVIPDiscounted ? vipDiscountedPrice.toFixed(2) : pkg.price.toFixed(2)}
                        {pkg.originalPrice && (
                      ...
                      {(pkg.discount || isVIPDiscounted) && (
                        <Badge variant="destructive">
                          {pkg.discount ? `${pkg.discount}% OFF` : 'VIP Discount'}
                        </Badge>
                      )}
                    </div>
                  </CardHeader>
                  ...
                  <CardContent className="space-y-4">
                    <div className="text-center">
                      ...
                      <div className="text-sm text-muted-foreground">Total Gold Coins</div>
              ...
              <CardContent className="space-y-4">
                    ...
                    return (
                      <Card 
                        ...
                        className={`cursor-pointer transition-all hover:shadow-md ${
                          selectedPayment?.id === method.id ? 'ring-2 ring-purple-500' : ''
                        }`}
                ...
                {selectedPayment && (
                  <Button 
                    ...
                    size="lg"
                  >
                    {processing ? 'Processing...' : `Complete Purchase - $${calculateVIPDiscount(selectedPackage.price).toFixed(2)}`}
                  </Button>
                )}
              </CardContent>
            </Card>
        ...
        <TabsContent value="custom" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Enter any amount between $5 and $500 to get Gold Coins and bonus rewards
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">
                          {calculateCustomPackage(parseFloat(customAmount)).goldCoins.toLocaleString()}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span>Bonus Gold Coins:</span>
                        <span className="font-medium text-green-600">
                          +{calculateCustomPackage(parseFloat(customAmount)).bonusGC.toLocaleString()}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span>Sweeps Coins:</span>
                        <span className="font-medium text-purple-600">
                          {calculateCustomPackage(parseFloat(customAmount)).sweepsCoins}
                        </span>
                      </div>
                      <div className="border-t pt-2">
                        <div className="flex justify-between font-bold">
                          ...
                          <span>
                            {(calculateCustomPackage(parseFloat(customAmount)).goldCoins + 
                              calculateCustomPackage(parseFloat(customAmount)).bonusGC).toLocaleString()} GC
                          </span>
                        </div>
                      </div>
                    </div>
                ...
                <div>
                  ...
                  {selectedPayment && (
                    <Button 
                      ...
                      size="lg"
                    >
                      {processing ? 'Processing...' : `Complete Purchase - $${calculateVIPDiscount(parseFloat(customAmount)).toFixed(2)}`}
                    </Button>
                  )}
                </div>
              )}
                  ...
                  {purchaseHistory.map((purchase) => (
                    <div key={purchase.id} className="flex items-center justify-between p-4 border rounded-lg">
                      ...
                      <div className="text-right">
                        ...
                        <Badge variant={purchase.status === 'completed' ? 'default' : 'destructive'}>
                          {purchase.status}
                        </Badge>
                      </div>
                    </div>
