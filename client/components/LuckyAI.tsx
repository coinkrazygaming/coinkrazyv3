...
const LuckyAI: React.FC = () => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      id: '1',
      content: 'Welcome to CoinKrazy! I\'m LuckyAI, your personal gaming assistant. How can I help you today?',
      timestamp: new Date(),
      sender: 'ai',
    },
  ]);
  ...
  const handleVMAction = (vmId: string, action: 'start' | 'stop' | 'pause' | 'restart') => {
    setVmInstances(prev =>
      prev.map(vm => {
        if (vm.id === vmId) {
        ...
        prev.map(vm => {
          if (vm.id === vmId) {
            switch (action) {
              ...
              case 'stop':
                return { ...vm, status: 'stopped' as const, cpu: 0, memory: 0, network: 0 };
              default:
                return vm;
            }
          }
        ...
        {!isMinimized && (
          <CardContent className="p-0 h-[calc(100%-4rem)]">
            <Tabs value={activeTab} onValueChange={setActiveTab} className="h-full">
              ...
              {/* Chat Tab */}
              <TabsContent value="chat" className="h-[calc(100%-3rem)] flex flex-col p-4">
                <div className="flex-1 overflow-y-auto space-y-4 mb-4">
                  {messages.map((message) => (
                    ...
                    >
                      ...
                      >
                        {message.type === 'bonus' && <Gift className="w-4 h-4 inline mr-2" />}
              ...
              {/* VM Manager Tab */}
              <TabsContent value="vm" className="h-[calc(100%-3rem)] overflow-y-auto p-4">
              ...
              {/* Bonuses Tab */}
              <TabsContent value="bonuses" className="h-[calc(100%-3rem)] overflow-y-auto p-4">
                <div className="space-y-4">
                  ...
                  {bonusOffers.map((bonus) => (
                    <Card key={bonus.id} className="p-4">
                      <div className="flex items-start justify-between mb-2">
                        <div className="flex items-center space-x-2">
                          {bonus.type === 'coins' && <Coins className="w-5 h-5 text-gold-500" />}
                          {bonus.type === 'sweeps' && <Star className="w-5 h-5 text-purple-500" />}
                          {bonus.type === 'multiplier' && <Zap className="w-5 h-5 text-orange-500" />}
                          <h4 className="font-medium">{bonus.title}</h4>
                        </div>
                        <Badge variant={bonus.claimed ? 'secondary' : 'default'}>
                      ...
                      <div className="text-sm space-y-1 mb-3">
                        <p>
                          ...
                          {bonus.value} {bonus.type === 'multiplier' ? 'x' : bonus.type}
                        </p>
                        <p>
                          <span className="font-medium">Expires:</span> {bonus.expires}
                        </p>
                        {bonus.requirements && (
                          <p>
                            <span className="font-medium">Requirements:</span> {bonus.requirements}
                          </p>
                        )}
                      </div>
              ...
              {/* Analytics Tab */}
              <TabsContent value="analytics" className="h-[calc(100%-3rem)] overflow-y-auto p-4">
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <Card className="p-3">
                      ...
                      <p className="text-2xl font-bold">{analytics.totalGamesPlayed.toLocaleString()}</p>
                    </Card>
                    ...
                    <Card className="p-3">
                      ...
                      <p className="text-2xl font-bold">{analytics.totalWins.toLocaleString()}</p>
                    </Card>
                    ...
                    <Card className="p-3">
                      ...
                      <p className="text-2xl font-bold">{analytics.totalCoinsWon.toLocaleString()}</p>
                    </Card>
                    ...
                    <Card className="p-3">
                      ...
                      <p className="text-2xl font-bold">{analytics.totalSweepsWon.toFixed(2)}</p>
                    </Card>
                  </div>
                  ...
                  <Card className="p-4">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-medium">{analytics.favoriteGame}</span>
                      </div>
                      ...
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-medium">#{analytics.rankPosition}</span>
                      </div>
                    </div>
                  </Card>
                </div>
              ...
              {/* Settings Tab */}
              <TabsContent value="settings" className="h-[calc(100%-3rem)] overflow-y-auto p-4">
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">Let AI play games automatically</p>
                      </div>
                      <Switch
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">Receive bonus and win alerts</p>
                      </div>
                      <Switch
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">Get game recommendations</p>
                      </div>
                      <Switch
                        checked={settings.aiAssistance}
                        onCheckedChange={(checked) => setSettings(prev => ({ ...prev, aiAssistance: checked }))}
                      />
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">Show personalized game suggestions</p>
                      </div>
                      <Switch
                        checked={settings.gameRecommendations}
                        onCheckedChange={(checked) => setSettings(prev => ({ ...prev, gameRecommendations: checked }))}
                      />
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">Alert when bonuses are available</p>
                      </div>
                      <Switch
                        checked={settings.bonusAlerts}
                        onCheckedChange={(checked) => setSettings(prev => ({ ...prev, bonusAlerts: checked }))}
                      />
                    </div>

                    <div>
                      <p className="font-medium mb-2">AI Responsiveness</p>
                      <p className="text-sm text-muted-foreground mb-2">How quickly AI responds to events</p>
                      <Slider
                        value={settings.responsiveness}
                        onValueChange={(value) => setSettings(prev => ({ ...prev, responsiveness: value }))}
                        max={5}
                        min={1}
                        step={1}
                        className="w-full"
