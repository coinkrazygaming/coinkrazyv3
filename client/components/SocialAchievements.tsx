...
export default function SocialAchievements() {
  ...
  const loadAchievementsData = async () => {
    ...
    try {
      ...
      // Mock daily rewards
      const mockDailyRewards: DailyReward[] = Array.from({ length: 7 }, (_, i) => ({
        day: i + 1,
        reward: {
          type: i === 6 ? 'SC' : 'GC',
  ...
  const getRarityColor = (rarity: string) => {
    switch (rarity) {
      case 'legendary': return 'from-yellow-500 to-gold-500';
  ...
  const getRarityBorder = (rarity: string) => {
    switch (rarity) {
      case 'legendary': return 'border-gold-500';
            ...
            <Card>
              <CardContent className="p-4 text-center">
                <div className="text-2xl font-bold text-gold-500">
                  {Math.round((badges.filter(b => b.unlockedAt).length / badges.length) * 100)}%
                </div>
          ...
          <Card>
            <CardHeader>
              ...
              <p className="text-sm text-muted-foreground">
                Login every day to claim increasing rewards. Miss a day and the streak resets!
              </p>
            </CardHeader>
            <CardContent>
                    ...
                    <div className="mb-3">
                      {reward.reward.type === 'GC' && (
                        <div className="flex flex-col items-center">
                          ...
                          <span className="text-sm font-medium">{reward.reward.amount} GC</span>
                        </div>
                      )}
                      {reward.reward.type === 'SC' && (
                        <div className="flex flex-col items-center">
                          <Crown className="h-6 w-6 text-purple-500 mb-1" />
                          <span className="text-sm font-medium">{reward.reward.amount} SC</span>
                        </div>
                      )}
                      {reward.reward.badge && (
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="text-center p-6 bg-gradient-to-br from-gold-500/20 to-yellow-500/20 rounded-lg">
                  ...
                  <div className="text-sm text-muted-foreground">Total GC Earned</div>
                </div>
                ...
                <div className="text-center p-6 bg-gradient-to-br from-purple-500/20 to-pink-500/20 rounded-lg">
                  ...
                  <div className="text-sm text-muted-foreground">Total SC Earned</div>
                </div>
        ...
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="flex items-center gap-3">
              ...
              {selectedBadge?.name}
              <span className="text-2xl">{selectedBadge && getBadgeIcon(selectedBadge.rarity)}</span>
            </DialogTitle>
