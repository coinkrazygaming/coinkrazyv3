...
export default function SocialActivitiesFeed() {
    ...
    const unsubscribe = socialService.onActivity((activity) => {
      ...
      setTimeout(() => {
        setActivities(prev => prev.map(a => 
          a.id === activity.id ? { ...a, isNew: false, animation: undefined } : a
        ));
      }, 3000);
    });
  ...
  const getActivityColor = (type: string, metadata?: any) => {
    switch (type) {
      case 'win':
        return metadata?.isJackpot ? 'from-gold-500/20 to-yellow-500/20' : 'from-green-500/20 to-emerald-500/20';
      case 'achievement':
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start gap-2">
                      ...
                      <div className="flex-1">
                        ...
                        {activity.metadata && (
                          <div className="mt-2">
                            {activity.type === 'win' && activity.metadata.winAmount && (
                              <div className="flex items-center gap-2">
                                <Badge className={activity.metadata.isJackpot ? 'bg-gold-500' : 'bg-green-500'}>
                                  {activity.metadata.isJackpot && <Crown className="h-3 w-3 mr-1" />}
                                  +{activity.metadata.winAmount.toLocaleString()} GC
                                </Badge>
                            ...
                            {activity.type === 'level_up' && activity.metadata.newLevel && (
                            ...
                            {activity.type === 'achievement' && activity.metadata.achievementName && (
                            ...
                            {activity.type === 'challenge_completed' && activity.metadata.reward && (
