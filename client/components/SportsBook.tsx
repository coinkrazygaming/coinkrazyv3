...
export default function SportsBook() {
  ...
  const [selectedCategory, setSelectedCategory] = useState<'all' | 'live' | 'upcoming' | 'popular'>('all');
  const [searchQuery, setSearchQuery] = useState('');
  const [betSlip, setBetSlip] = useState<BetSlip>({ selections: [], betType: 'single', totalStake: 0, totalOdds: 1, potentialPayout: 0 });
  ...
  const addToBetSlip = (event: SportEvent, marketName: string, outcomeName: string, odds: number, line?: number) => {
  ...
  const getSportIcon = (sport: SportType) => {
    switch (sport) {
      case 'football': return <Football className="w-5 h-5" />;
      case 'basketball': return <Target className="w-5 h-5" />;
      case 'baseball': return <Target className="w-5 h-5" />;
      case 'hockey': return <Target className="w-5 h-5" />;
      case 'soccer': return <Football className="w-5 h-5" />;
      case 'tennis': return <Target className="w-5 h-5" />;
      case 'golf': return <Target className="w-5 h-5" />;
      case 'mma': return <Target className="w-5 h-5" />;
      case 'boxing': return <Target className="w-5 h-5" />;
      ...
      <Card className={`hover:shadow-lg transition-all duration-300 ${
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                <div className="font-bold text-sm text-muted-foreground">{event.league}</div>
                <div className="text-xs text-muted-foreground">
                  {event.live ? `${event.period} - ${event.clock}` : formatTimeUntil(event.startTime)}
                </div>
              </div>
            </div>
        ...
        <CardContent className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                {event.awayTeam.record && (
                  <div className="text-xs text-muted-foreground">({event.awayTeam.record})</div>
                )}
              </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                {event.homeTeam.record && (
                  <div className="text-xs text-muted-foreground">({event.homeTeam.record})</div>
                )}
              </div>
          ...
          <div className="space-y-3">
            ...
            {moneylineMarket && (
              <div>
                <div className="text-xs text-muted-foreground mb-2">Moneyline</div>
                <div className="grid grid-cols-2 gap-2">
                  {moneylineMarket.outcomes.map(outcome => (
                    <Button
                      ...
                      onClick={() => addToBetSlip(event, 'Moneyline', outcome.name, outcome.odds)}
                      disabled={!outcome.isAvailable}
                    >
                      <div className="text-xs">{outcome.name}</div>
                      <div className="font-bold">{formatOdds(outcome.odds)}</div>
                    </Button>
                  ))}
                </div>
              </div>
            ...
            {spreadMarket && (
              <div>
                <div className="text-xs text-muted-foreground mb-2">Point Spread</div>
                <div className="grid grid-cols-2 gap-2">
                  {spreadMarket.outcomes.map(outcome => (
                    <Button
                      ...
                      size="sm"
                      className="flex flex-col h-auto p-2"
                      onClick={() => addToBetSlip(event, 'Point Spread', `${outcome.name} ${outcome.line}`, outcome.odds, outcome.line)}
                      disabled={!outcome.isAvailable}
                    >
                      <div className="text-xs">{outcome.name}</div>
                      <div className="font-bold">
                        {outcome.line && outcome.line > 0 ? '+' : ''}{outcome.line} ({formatOdds(outcome.odds)})
                      </div>
                    </Button>
                  ))}
                </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {totalMarket.outcomes.map(outcome => (
                    <Button
                      ...
                      size="sm"
                      className="flex flex-col h-auto p-2"
                      onClick={() => addToBetSlip(event, 'Total Points', `${outcome.name} ${outcome.total}`, outcome.odds, outcome.total)}
                      disabled={!outcome.isAvailable}
                    >
                      <div className="text-xs">{outcome.name}</div>
                      <div className="font-bold">
          ...
          <>
                  ...
                  {betSlip.betType === 'single' && (
                    <div className="mt-2">
                      <Input
                        ...
                        onChange={(e) => setStake(Number(e.target.value), selection.id)}
                        className="h-8"
                      />
                      {selection.stake && (
                        <div className="text-xs text-muted-foreground mt-1">
                          To win: {formatCurrency((selection.stake || 0) * selection.odds)}
                        </div>
                      )}
                    </div>
                  )}
            ...
            <div className="border-t pt-3 space-y-2">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-bold">{formatCurrency(betSlip.totalStake)}</span>
              </div>
              
              {betSlip.betType === 'parlay' && (
                <div className="flex justify-between text-sm">
                  <span>Total Odds:</span>
                  <span className="font-bold">{formatOdds(betSlip.totalOdds)}</span>
                </div>
              )}
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-muted-foreground mt-2">
                Live sports betting with real-time odds and comprehensive markets
              </p>
            </div>
        ...
        <CardContent className="p-6">
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <div className="flex gap-2">
              ...
              <Select value={oddsFormat} onValueChange={(value) => setOddsFormat(value as any)}>
              ...
              <Button variant="outline" onClick={() => setShowPromotionsDialog(true)}>
          ...
          {stats && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              <Card>
                ...
                <CardContent className="space-y-3">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-bold text-gold-400">{formatCurrency(stats.biggestWin)}</span>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent className="space-y-3">
                  {Object.entries(stats.sportBreakdown).slice(0, 5).map(([sport, data]) => (
              ...
              <Card>
                ...
                <CardContent className="space-y-3">
                  ...
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Parlay Accuracy:</span>
                        ...
                        <div>
                          ...
                          <span className="font-bold">
                            {promo.type === 'multiplier' ? `${promo.value}x` : 
                             promo.type === 'cashback' ? `${promo.value * 100}%` :
                             `${promo.value}`}
                          </span>
                        </div>
                        {promo.minBet && (
                          <div>
                            ...
                            <span className="font-bold">{formatCurrency(promo.minBet)}</span>
                          </div>
                        )}
                        {promo.maxBenefit && (
                          <div>
                            <span className="text-muted-foreground">Max Benefit: </span>
                            <span className="font-bold">{formatCurrency(promo.maxBenefit)}</span>
                          </div>
                        )}
                      </div>
