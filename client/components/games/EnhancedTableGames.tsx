...
import { gameInterfaceService, TableGameResult } from '../../services/gameInterfaceService';
...
const EnhancedTableGames: React.FC = () => {
  ...
  const handleGameSelect = async (game: TableGame) => {
    ...
    try {
      await gameInterfaceService.startGameSession(userId, 'table', selectedCurrency);
  ...
  const handleBetChange = (change: number) => {
    ...
    const newBet = Math.max(currentGame.minBet[selectedCurrency], betAmount + change);
  ...
  const startBlackjackHand = async () => {
    ...
    try {
      ...
      setBlackjackGame({
        ...
        canDouble: true,
        canSplit: false,
        result: playerCards[0][0] === 'A' && ['K', 'Q', 'J', '10'].includes(playerCards[1][0]) ? 'blackjack' : undefined,
      });
      ...
      if (blackjackGame.result === 'blackjack') {
        ...
        await gameInterfaceService.processGameResult(
          bet,
          'win',
          winAmount,
          { gameType: 'blackjack', result: 'blackjack' }
        );
        ...
        setSession(prev => ({
          ...
          winRate: ((prev.totalWin + winAmount) / (prev.totalBet + betAmount)) * 100,
        }));
  ...
  const blackjackHit = () => {
    // Simulate hitting
    const newCard = ['2H', '3S', '4D', '5C', '6H'][Math.floor(Math.random() * 5)];
    ...
    if (newValue > 21) {
      setBlackjackGame(prev => ({ ...prev, result: 'lose', gameStatus: 'finished' }));
      toast({
        title: 'Bust!',
  ...
  const blackjackStand = () => {
    ...
    setTimeout(() => {
      ...
      const result = blackjackGame.playerValue > dealerFinalValue || dealerFinalValue > 21 ? 'win' : 
                    blackjackGame.playerValue < dealerFinalValue ? 'lose' : 'push';
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {currentGame.isLive ? `Live Dealer: ${currentGame.dealer}` : 'RNG Table Game'}
                    </p>
                  </div>
                </div>
                    ...
                    {currentGame.type === 'blackjack' && (
                      <div className="text-white w-full h-full flex flex-col items-center justify-center">
                        <div className="text-center mb-8">
                          ...
                          {/* Dealer Hand */}
                          <div className="mb-6">
                            <p className="text-sm mb-2">Dealer ({blackjackGame.dealerValue})</p>
                          ...
                          {/* Player Hand */}
                          <div>
                            <p className="text-sm mb-2">Player ({blackjackGame.playerValue})</p>
                            <div className="flex justify-center gap-2">
                          ...
                          {blackjackGame.result && (
                            <div className="mt-4 text-xl font-bold">
                              {blackjackGame.result === 'win' && <span className="text-green-400">YOU WIN!</span>}
                              {blackjackGame.result === 'lose' && <span className="text-red-400">DEALER WINS</span>}
                              {blackjackGame.result === 'push' && <span className="text-yellow-400">PUSH</span>}
                              {blackjackGame.result === 'blackjack' && <span className="text-gold-400">BLACKJACK!</span>}
                            </div>
                          )}
                        </div>
                      </div>
                    ...
                    {currentGame.type === 'roulette' && (
                      <div className="text-white w-full h-full flex flex-col items-center justify-center">
                        <div className="text-center">
                          <h3 className="text-2xl font-bold mb-4">European Roulette</h3>
                          
                          {/* Roulette Wheel */}
                          <div className={`w-32 h-32 border-4 border-gold-500 rounded-full mx-auto mb-4 flex items-center justify-center ${rouletteGame.spinning ? 'animate-spin' : ''}`}>
                            <div className="text-3xl font-bold">
                              {rouletteGame.spinning ? '🎰' : rouletteGame.result !== undefined ? rouletteGame.result : '?'}
                            </div>
                          </div>
                                ...
                                return (
                                  <div
                                    ...
                                    className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-sm font-bold ${
                                      num === 0 ? 'bg-green-600' : isRed ? 'bg-red-600' : 'bg-black'
                                    }`}
                                  >
                                    {num}
                                  </div>
                      ...
                      <CardContent className="p-4">
                        ...
                        <div className="flex items-center gap-2 mb-3">
                          <Button
                            ...
                            disabled={betAmount <= currentGame.minBet[selectedCurrency]}
                          >
                          ...
                          <Input
                            type="number"
                            value={betAmount}
                            onChange={(e) => setBetAmount(Math.max(currentGame.minBet[selectedCurrency], parseFloat(e.target.value) || currentGame.minBet[selectedCurrency]))}
                      ...
                      <CardContent className="p-4">
                        ...
                        <div className="space-y-2 text-sm">
                          <div className="flex justify-between">
                            <span>Hands:</span>
                            <span className="font-medium">{session.handsPlayed}</span>
                          </div>
                          <div className="flex justify-between">
                            <span>Net:</span>
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-muted-foreground">Classic casino table games with live dealers</p>
          </div>
              ...
              <CardContent className="p-4">
                ...
                <div className="flex items-center justify-between text-xs">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span className="font-medium">{game.maxBet[selectedCurrency]} {selectedCurrency}</span>
                  </div>
                </div>
