...
const blockedStates = [
  {
    code: "WA",
    name: "Washington",
    reason: "State gambling laws prohibit sweepstakes casinos",
  },
  {
    code: "ID",
    name: "Idaho",
    reason: "Sweepstakes model not permitted under state law",
  },
  {
    code: "MT",
    name: "Montana",
    reason: "Regulatory restrictions on online gambling",
  },
  {
    code: "NV",
    name: "Nevada",
    reason: "Exclusive licensed casino regulations",
  },
...
const complianceRules = [
  {
    id: 1,
    rule: "Minimum Age Verification",
    status: "active",
    description: "All users must be 18+ (21+ in some states)",
  },
  {
    id: 2,
    rule: "State Geo-blocking",
    status: "active",
    description: "Block access from prohibited states",
  },
  {
    id: 3,
    rule: "KYC Verification",
    status: "active",
    description: "Identity verification for SC withdrawals",
  },
  {
    id: 4,
    rule: "PEP Screening",
  ...
  },
  {
    ...
    status: "active",
    description: "AML monitoring and reporting",
  },
  {
    id: 7,
    rule: "Responsible Gaming",
    status: "active",
    description: "Player protection measures",
  },
  {
    id: 8,
  ...
  const [auditLog] = useState([
    {
      ...
      action: "User blocked",
      reason: "Washington state access attempt",
      user: "user123",
      severity: "high",
    },
    {
  ...
  useEffect(() => {
    ...
    const detectLocation = async () => {
      try {
        ...
        const isBlocked = blockedStates.some(
          (state) => state.code === mockLocation.state,
        );
    ...
    <div className="min-h-screen bg-background">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-muted-foreground">
                Legal compliance, state blocking, and age verification
              </p>
            </div>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          <Card
            className={`${complianceStatus.ageVerified ? "bg-green-500/5 border-green-500/20" : "bg-orange-500/5 border-orange-500/20"}`}
          >
            <CardContent className="p-4 text-center">
              <Calendar
                className={`w-8 h-8 mx-auto mb-2 ${complianceStatus.ageVerified ? "text-green-500" : "text-orange-500"}`}
              />
              <div className="text-lg font-bold">Age Verification</div>
              <Badge
                variant={complianceStatus.ageVerified ? "default" : "secondary"}
              >
          ...
          <Card
            className={`${complianceStatus.locationAllowed ? "bg-green-500/5 border-green-500/20" : "bg-red-500/5 border-red-500/20"}`}
          >
            <CardContent className="p-4 text-center">
              <MapPin
                className={`w-8 h-8 mx-auto mb-2 ${complianceStatus.locationAllowed ? "text-green-500" : "text-red-500"}`}
              />
              <div className="text-lg font-bold">Location Check</div>
              <Badge
                variant={
                  complianceStatus.locationAllowed ? "default" : "destructive"
                }
              >
                {userLocation
          ...
          <Card
            className={`${complianceStatus.kycCompleted ? "bg-green-500/5 border-green-500/20" : "bg-orange-500/5 border-orange-500/20"}`}
          >
            <CardContent className="p-4 text-center">
              <Shield
                className={`w-8 h-8 mx-auto mb-2 ${complianceStatus.kycCompleted ? "text-green-500" : "text-orange-500"}`}
              />
              <div className="text-lg font-bold">KYC Status</div>
              <Badge
                variant={
                  complianceStatus.kycCompleted ? "default" : "secondary"
                }
              >
          ...
          <Card
            className={`${complianceStatus.sanctionsCleared ? "bg-green-500/5 border-green-500/20" : "bg-orange-500/5 border-orange-500/20"}`}
          >
            <CardContent className="p-4 text-center">
              <Scale
                className={`w-8 h-8 mx-auto mb-2 ${complianceStatus.sanctionsCleared ? "text-green-500" : "text-orange-500"}`}
              />
              <div className="text-lg font-bold">Sanctions Check</div>
              <Badge
                variant={
                  complianceStatus.sanctionsCleared ? "default" : "secondary"
                }
              >
                ...
                <CardContent>
                  <div className="space-y-4">
                    <div className="space-y-3">
                      ...
                      <div className="flex items-center gap-3 p-3 bg-muted/20 rounded-lg">
                        <div className="w-8 h-8 bg-casino-blue rounded-full flex items-center justify-center text-white font-bold text-sm">
                          2
                        </div>
                        <div>
