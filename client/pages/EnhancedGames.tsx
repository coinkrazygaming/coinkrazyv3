...
const EnhancedGames: React.FC = () => {
  ...
  const gameCategories: GameCategory[] = [
    ...
    },
    {
      ...
      description: 'Classic casino games like Blackjack, Roulette, and Baccarat',
      playerCount: 423,
      gameCount: 24,
      currency: ['GC', 'SC'],
      status: 'active',
  ...
  const loadGameData = async () => {
    ...
    try {
      ...
      setQuickStats({
        ...
        activeGames: gameCategories.reduce((sum, cat) => sum + cat.gameCount, 0),
  ...
  const handleCategorySelect = (categoryId: string) => {
    ...
    if (category && category.currency.length === 1) {
      ...
      gameInterfaceService.setUserCurrency(userId, category.currency[0], categoryId as any);
    }
  };
          ...
          <div className="container mx-auto px-4">
            <div className="flex items-center justify-between">
              ...
              <CurrencySelector
                ...
                compact={true}
                showSettings={false}
                allowedCurrencies={gameCategories.find(cat => cat.id === activeTab)?.currency}
              />
            </div>
          </div>
        </div>
        ...
        <div className="container mx-auto px-4">
          <div className="text-center mb-8">
            ...
            <p className="text-muted-foreground text-lg">
              Experience the thrill of casino gaming with our extensive collection
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
            <Card className="text-center p-4">
              ...
              <div className="text-2xl font-bold">{quickStats.totalPlayers.toLocaleString()}</div>
            ...
            <Card className="text-center p-4">
              ...
              <div className="text-2xl font-bold">{formatCurrency(quickStats.totalWinnings)}</div>
              <div className="text-xs text-muted-foreground">Won Today</div>
            </Card>

            <Card className="text-center p-4">
              <Crown className="w-8 h-8 text-purple-500 mx-auto mb-2" />
              <div className="text-2xl font-bold">{formatCurrency(quickStats.jackpotPool)}</div>
              <div className="text-xs text-muted-foreground">Jackpot Pool</div>
            </Card>

            <Card className="text-center p-4">
              <Flame className="w-8 h-8 text-orange-500 mx-auto mb-2" />
              <div className="text-lg font-bold truncate">{quickStats.hotGame}</div>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-12">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-muted-foreground">Our most popular and newest games</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {gameCategories.map((category) => (
              <Card
                ...
                onClick={() => category.status === 'active' && handleCategorySelect(category.id)}
              >
                <CardContent className="p-6">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3">
                      <div className={`p-3 rounded-lg ${
                        category.id === 'slots' ? 'bg-gold-500/10 text-gold-500' :
                        category.id === 'table' ? 'bg-green-500/10 text-green-500' :
                        category.id === 'live' ? 'bg-red-500/10 text-red-500' :
                        category.id === 'bingo' ? 'bg-purple-500/10 text-purple-500' :
                        category.id === 'sportsbook' ? 'bg-blue-500/10 text-blue-500' :
                    ...
                    <div className="text-right">
                      <div className="flex items-center gap-1 text-sm">
                        ...
                        <span className="font-medium">{category.playerCount}</span>
                      </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-medium">
                        {category.status === 'active' ? 'Available' :
                         category.status === 'maintenance' ? 'Maintenance' : 'Coming Soon'}
                      </span>
                    </div>
        ...
        <div className="mb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              { game: 'CoinKrazy Special', amount: 15247.50, player: 'Player***23', time: '2 minutes ago' },
              { game: 'Live Blackjack VIP', amount: 8950.00, player: 'Winner***89', time: '8 minutes ago' },
              { game: 'Mega Bingo', amount: 3420.75, player: 'Lucky***45', time: '15 minutes ago' },
            ].map((winner, index) => (
              <Card key={index} className="bg-gradient-to-r from-gold/5 to-gold/10 border-gold-500/20">
                <CardContent className="p-4">
                  <div className="flex items-center gap-3">
                    <Trophy className="w-8 h-8 text-gold-500" />
                    <div className="flex-1">
                      <h4 className="font-bold">{winner.game}</h4>
                      <p className="text-sm text-muted-foreground">{winner.player}</p>
                    </div>
                    <div className="text-right">
                      <p className="font-bold text-gold-500">{formatCurrency(winner.amount)}</p>
        ...
        {platformStats && (
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <Card className="text-center p-6">
              ...
              <div className="text-2xl font-bold">{platformStats.totalGamesAvailable}</div>
              <div className="text-sm text-muted-foreground">Total Games</div>
            </Card>

            <Card className="text-center p-6">
              <TrendingUp className="w-12 h-12 text-green-500 mx-auto mb-4" />
              <div className="text-2xl font-bold">{platformStats.totalSCWonToday.toFixed(2)}</div>
