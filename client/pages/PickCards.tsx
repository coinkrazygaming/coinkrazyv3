...
export default function PickCards() {
  ...
  const calculateStats = (cards: PickCard[]) => {
    ...
    const completedBets = cards.filter(card => card.status === 'won' || card.status === 'lost');
    const wonBets = cards.filter(card => card.status === 'won');
    const totalWagered = cards.filter(card => card.status !== 'building').reduce((sum, card) => sum + card.wagerAmount, 0);
    const totalWon = wonBets.reduce((sum, card) => sum + card.potentialPayout, 0);
  ...
  const handleUpdateCard = (updatedCard: PickCard) => {
    setPickCards(prev => prev.map(card => 
  ...
  const handlePlaceBet = (card: PickCard) => {
    ...
    setPickCards(prev => prev.map(c => c.id === card.id ? placedCard : c));
  ...
  const filteredCards = pickCards.filter(card => {
    switch (activeTab) {
      case 'building': return card.status === 'building';
      case 'active': return card.status === 'placed';
      case 'completed': return card.status === 'won' || card.status === 'lost';
      case 'saved': return card.status === 'saved';
      default: return true;
    }
  });
        ...
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-muted-foreground">Build your parlay picks and track your betting performance</p>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          <Card className="bg-gradient-to-br from-casino-blue/10 to-casino-blue/5 border-casino-blue/20">
            <CardContent className="p-4 text-center">
              ...
              <p className="text-sm text-muted-foreground">Both picks must win</p>
            </CardContent>
          </Card>
          ...
          <Card className="bg-gradient-to-br from-gold/10 to-gold/5 border-gold-500/20">
            <CardContent className="p-4 text-center">
              ...
              <p className="text-sm text-muted-foreground">All 3 picks must win</p>
            </CardContent>
          </Card>
          ...
          <Card className="bg-gradient-to-br from-purple-500/10 to-purple-500/5 border-purple-500/20">
            <CardContent className="p-4 text-center">
              ...
              <p className="text-sm text-muted-foreground">All 4 picks must win</p>
            </CardContent>
          </Card>
        </div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="grid w-full grid-cols-5">
            <TabsTrigger value="building">
              <Plus className="w-4 h-4 mr-2" />
              Building ({pickCards.filter(c => c.status === 'building').length})
            </TabsTrigger>
            ...
            <TabsTrigger value="completed">
              ...
              Completed ({pickCards.filter(c => c.status === 'won' || c.status === 'lost').length})
            </TabsTrigger>
              ...
              <Card className="text-center p-12">
                <CardContent>
                  ...
                  <h3 className="text-xl font-bold mb-2">
                    {activeTab === 'building' ? 'No Pick Cards in Progress' :
                     activeTab === 'active' ? 'No Active Bets' :
                     activeTab === 'completed' ? 'No Completed Bets' :
                     activeTab === 'saved' ? 'No Saved Cards' :
                     'No Pick Cards Yet'}
                  </h3>
                  <p className="text-muted-foreground mb-6">
                    {activeTab === 'building' ? 'Create a new pick card to start building your parlay bets.' :
                     activeTab === 'active' ? 'Place some bets to see them here!' :
                     activeTab === 'completed' ? 'Complete some bets to see your results here.' :
                     activeTab === 'saved' ? 'Save pick cards to your cart when you need more coins.' :
