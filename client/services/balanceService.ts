...
class BalanceService {
  ...
  private listeners: Map<string, Set<(balance: UserBalance) => void>> = new Map();
  ...
  private simulateBalanceUpdates() {
    ...
    randomUsers.forEach(userId => {
      ...
      if (balance) {
        ...
        const scChange = Math.random() > 0.9 ? (Math.random() > 0.5 ? 1 : -1) : 0; // Rare SC changes
  ...
  getUserBalance(userId: string): UserBalance {
    ...
    if (!balance) {
      ...
      this.addTransaction(userId, 'credit', 'gc', 50000, 'Welcome Bonus - Gold Coins', undefined, balance.gc);
      this.addTransaction(userId, 'credit', 'sc', 25, 'Welcome Bonus - Sweeps Coins', undefined, balance.sc);
    }
    return balance;
  }

  updateBalance(userId: string, gcChange: number, scChange: number, description: string, gameId?: string): UserBalance {
  ...
  getUserTransactions(userId: string, limit: number = 20): BalanceTransaction[] {
    return this.transactions.get(userId)?.slice(0, limit) || [];
  }

  subscribeToBalanceUpdates(userId: string, callback: (balance: UserBalance) => void): () => void {
  ...
  awardFirstDepositBonus(userId: string, depositAmount: number): UserBalance {
    ...
    return this.updateBalance(userId, 0, scAwarded, `First Deposit Bonus - ${matchAmount}% match on $${depositAmount}`);
  }

  awardMiniGameReward(userId: string, gameId: string, amount: number): UserBalance {
    const maxDaily = 0.25;
    ...
    return this.updateBalance(userId, 0, actualAmount, `Mini Game Reward - ${gameId}`, gameId);
  }

  // Admin methods
  getAllBalances(): UserBalance[] {
    return Array.from(this.balances.values()).sort((a, b) => 
