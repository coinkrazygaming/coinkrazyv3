...
class BingoService {
  ...
  private initializeWebSocket() {
    ...
    try {
      ...
      console.warn('Bingo WebSocket initialization failed, using mock data:', error);
      this.loadMockData();
      this.simulateRealTimeUpdates();
    }
  }
  ...
  private setupSocketListeners() {
    ...
    this.socket.on('game_start', (data: { roomId: string; gameNumber: number }) => {
    ...
    this.socket.on('number_called', (data: { roomId: string; number: number }) => {
      const room = this.rooms.get(data.roomId);
    ...
    const mockRooms: BingoRoom[] = [
      {
        ...
        theme: "classic-gold",
        featured: true,
        description: "The premium 75-ball bingo experience with huge progressive jackpots"
      },
      {
        id: "silver-90",
        name: "Silver Lounge",
        ...
        theme: "silver-modern",
        featured: false,
        description: "Classic 90-ball bingo with traditional patterns and steady payouts"
      },
      {
        id: "bronze-30",
        name: "Speed Zone",
        ...
        theme: "neon-speed",
        featured: false,
        description: "Fast-paced 30-ball bingo for quick wins and non-stop action"
      },
      {
        id: "vip-progressive",
        ...
        theme: "vip-luxury",
        featured: true,
        description: "Exclusive high-stakes bingo with massive progressive jackpots"
      },
      {
        ...
        featured: true,
        description: "Competitive tournament-style bingo with elimination rounds"
      }
    ];
    ...
    const mockTournaments: BingoTournament[] = [
      {
        ...
        currentRound: 0,
        winners: [],
        description: "Single elimination tournament with massive prizes for the best bingo players"
      },
      {
        ...
        currentRound: 0,
        winners: [],
        description: "Progressive tournament where prize pools grow with each elimination"
      }
    ];
  ...
  private simulateRealTimeUpdates() {
    setInterval(() => {
      ...
      this.rooms.forEach(room => {
        ...
        room.currentPlayers = Math.max(0, Math.min(room.maxPlayers, room.currentPlayers + change));
  ...
  private simulateNumberCall(roomId: string) {
    ...
    const maxNumber = room.type === "75-ball" ? 75 : room.type === "90-ball" ? 90 : 30;
  ...
  private simulateRandomChatMessage() {
    ...
    const usernames = ["BingoKing", "LuckyLady", "NumberMaster", "BingoQueen", "WinnerWinner"];
  ...
  private checkAutoMark(roomId: string, number: number) {
    this.playerCards.forEach(card => {
      if (card.autoMark && card.active) {
        const flatNumbers = card.numbers.flat().filter(n => n !== null) as number[];
  ...
  public generateBingoCard(type: "30-ball" | "75-ball" | "90-ball", cost: { gc: number; sc: number }): BingoCard {
    ...
    switch (type) {
      ...
      case "75-ball":
        ...
        patterns = ["line-horizontal", "four-corners", "x-pattern", "full-house"];
        break;
      case "90-ball":
  ...
  public sendChatMessage(roomId: string, message: string, username: string): void {
  ...
  public checkForWin(cardId: string, pattern: BingoPattern): boolean {
    ...
    for (let row = 0; row < rows && row < pattern.pattern.length; row++) {
      for (let col = 0; col < cols && col < pattern.pattern[row].length; col++) {
