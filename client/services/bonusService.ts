...
class BonusService {
      ...
      if (bonus.rows.length === 0) {
        ...
        `,
          [
            ...
            10,
            10,
            JSON.stringify({ requires_email_verification: true }),
          ],
        );
      }
  ...
  async createDepositBonus(
    ...
    bonusPercentage: number = 100,
  ): Promise<Bonus> {
    try {
      const maxBonus = 100; // Maximum $100 bonus
      const bonusAmount = Math.min(
        depositAmount * (bonusPercentage / 100),
        maxBonus,
      );
  ...
  async checkLoyaltyBonus(
    userId: number,
  ): Promise<{ eligible: boolean; nextTier?: string; progress?: number }> {
    try {
      ...
      const totalWagered = parseFloat(
        wageringResult.rows[0]?.total_wagered || "0",
      );
  ...
  ): Promise<{ referrerBonus: Bonus; referredBonus: Bonus }> {
    try {
  ...
  async getUserBonusHistory(userId: number): Promise<BonusHistory> {
    try {
      ...
      const totalGCAwarded = bonuses.reduce(
        (sum, bonus) => sum + (bonus.gc_amount || 0),
        0,
      );
      const totalSCAwarded = bonuses.reduce(
        (sum, bonus) => sum + (bonus.sc_amount || 0),
        0,
      );
      const totalClaimed = bonuses.filter(
        (bonus) => bonus.status === "claimed",
      ).length;
    ...
    try {
      ...
      if (bonus.expires_at && new Date(bonus.expires_at) < new Date()) {
        await databaseService.query(
          "UPDATE bonuses SET status = 'expired' WHERE id = $1",
          [bonusId],
        );
