...
export interface Bonus {
  id: number;
  user_id: number;
  bonus_type: 'welcome' | 'daily' | 'deposit' | 'loyalty' | 'referral' | 'social' | 'vip' | 'wheel' | 'birthday';
  gold_coins: number;
  sweeps_coins: number;
  description: string;
  requirements?: string;
...
export const VIP_TIERS: VIPTier[] = [
  ...
  },
  {
    ...
    perks: ["Priority support", "Exclusive tournaments", "Higher daily bonuses"],
    color: "text-gray-400"
  },
  {
    level: 2,
    ...
    perks: ["VIP support line", "Exclusive games", "Birthday bonus", "Weekly cashback"],
    color: "text-gold-500"
  },
  {
    level: 3,
    ...
    perks: ["Personal account manager", "Luxury gifts", "Custom bonuses", "Instant withdrawals"],
  ...
  },
  {
    ...
    perks: ["Ultra VIP treatment", "Exclusive events", "Maximum bonuses", "White-glove service"],
    color: "text-blue-400"
  }
];
...
class BonusService {
  ...
  async claimDailyBonus(userId: number): Promise<{ success: boolean; message: string; bonus?: Bonus }> {
    try {
  ...
  async claimLoyaltyBonus(userId: number): Promise<{ success: boolean; message: string; bonus?: Bonus }> {
    try {
  ...
  async claimReferralBonus(userId: number, referredUserId: number): Promise<{ success: boolean; message: string }> {
    try {
      ...
      const existingReferralBonus = await this.getReferralBonus(userId, referredUserId);
  ...
  async claimWheelSpinBonus(userId: number, scWon: number): Promise<{ success: boolean; message: string }> {
    try {
  ...
  async claimVIPBonus(userId: number): Promise<{ success: boolean; message: string; bonus?: Bonus }> {
    try {
      const vipTier = await this.getUserVIPTier(userId);
  ...
  async getBonusProgress(userId: number): Promise<BonusProgress[]> {
    try {
      ...
      progress.push({
        ...
        reward: { gc: 5000, sc: 5 },
        next_claim: lastDailyBonus ? this.getNextDailyBonusTime() : 'Available now'
      });
      ...
      if (nextTier) {
        progress.push({
          ...
          reward: { gc: nextTier.monthly_bonus_gc, sc: nextTier.monthly_bonus_sc }
        });
      }
  ...
  private async createBonus(userId: number, bonusData: Partial<Bonus>): Promise<Bonus> {
  ...
  private async claimBonus(bonusId: number, userId: number): Promise<{ success: boolean; message: string }> {
    try {
  ...
  private async getUserBonuses(userId: number, type?: string): Promise<Bonus[]> {
  ...
  private async deductLoyaltyPoints(userId: number, points: number): Promise<void> {
    // In a real implementation, this would update the database
  }

  private async getReferralBonus(userId: number, referredUserId: number): Promise<Bonus | null> {
