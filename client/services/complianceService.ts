...
const RESTRICTED_STATES = [
  'NY', 'FL', 'ND', 'TN', 'RI', 'CT', 'MD', 'MT', 'VT', 'WA'
];

// Countries where sweepstakes are not allowed
const RESTRICTED_COUNTRIES = [
  'CN', 'RU', 'IR', 'KP', 'SY', 'CU', 'MM', 'AF', 'BY', 'LY'
];

// Minimum age requirements by state
const STATE_AGE_REQUIREMENTS: Record<string, number> = {
  'AL': 19, 'AK': 18, 'AZ': 18, 'AR': 18, 'CA': 18, 'CO': 18,
  'CT': 18, 'DE': 18, 'FL': 18, 'GA': 18, 'HI': 18, 'ID': 18,
  'IL': 18, 'IN': 18, 'IA': 18, 'KS': 18, 'KY': 18, 'LA': 18,
  'ME': 18, 'MD': 18, 'MA': 18, 'MI': 18, 'MN': 18, 'MS': 18,
  'MO': 18, 'MT': 18, 'NE': 19, 'NV': 18, 'NH': 18, 'NJ': 18,
  'NM': 18, 'NY': 18, 'NC': 18, 'ND': 18, 'OH': 18, 'OK': 18,
  'OR': 18, 'PA': 18, 'RI': 18, 'SC': 18, 'SD': 18, 'TN': 18,
  'TX': 18, 'UT': 18, 'VT': 18, 'VA': 18, 'WA': 18, 'WV': 18,
  'WI': 18, 'WY': 18
};
...
class ComplianceService {
  ...
  async verifyLocation(): Promise<{ state: string; country: string; isRestricted: boolean }> {
    try {
  ...
  async checkCompliance(userId?: string): Promise<ComplianceData> {
    ...
    try {
      ...
      const response = await fetch(`${this.baseUrl}/check${userId ? `?userId=${userId}` : ''}`, {
      ...
      if (!contentType || !contentType.includes('application/json')) {
        console.warn('Compliance check response is not JSON, using fallback data');
        return fallbackData;
      }
  ...
  async getSweepstakesRules(): Promise<SweepstakesRules[]> {
    const fallbackRules: SweepstakesRules[] = [
      {
        ...
        prizeStructure: [
          { tier: 'Grand Prize', description: 'Cash Prize', value: '$5,000', quantity: 1 },
          { tier: 'First Prize', description: 'Cash Prize', value: '$1,000', quantity: 5 },
          { tier: 'Second Prize', description: 'Cash Prize', value: '$500', quantity: 10 },
          { tier: 'Third Prize', description: 'Cash Prize', value: '$100', quantity: 50 }
        ],
        drawDates: ['Daily at 11:59 PM EST'],
        officialRules: 'Complete official rules available at coinkrazy.com/rules',
        noPurchaseNecessary: 'NO PURCHASE NECESSARY TO ENTER OR WIN. A PURCHASE WILL NOT IMPROVE YOUR CHANCES OF WINNING.',
        oddsDisclosure: 'Odds of winning depend on number of eligible entries received',
    ...
    try {
      ...
      const contentType = response.headers.get('content-type');
      if (!contentType || !contentType.includes('application/json')) {
        console.warn('Sweepstakes rules response is not JSON, using fallback data');
        return fallbackRules;
      }
      ...
      console.warn('Failed to fetch sweepstakes rules, using fallback data:', error);
      return fallbackRules;
    }
  }
  ...
  async getComplianceAlerts(): Promise<ComplianceAlert[]> {
    ...
    try {
      ...
      const contentType = response.headers.get('content-type');
      if (!contentType || !contentType.includes('application/json')) {
        console.warn('Compliance alerts response is not JSON, using fallback data');
        return fallbackAlerts;
      }
      ...
      console.warn('Failed to fetch compliance alerts, using fallback data:', error);
      return fallbackAlerts;
    }
  }
  ...
  async getComplianceStatus(): Promise<{
    ...
    try {
      ...
      const contentType = response.headers.get('content-type');
      if (!contentType || !contentType.includes('application/json')) {
        console.warn('Compliance status response is not JSON, using fallback data');
        return fallbackStatus;
      }
      ...
      console.warn('Failed to fetch compliance status, using fallback data:', error);
      return fallbackStatus;
    }
  }

  // Responsible Gaming Features
  async setPlayLimits(userId: string, limits: {
  ...
  // Self-Exclusion
  async requestSelfExclusion(userId: string, duration: '24h' | '7d' | '30d' | '6m' | '1y' | 'permanent'): Promise<boolean> {
    try {
      const controller = new AbortController();
      const timeoutId = setTimeout(() => controller.abort(), 3000);
