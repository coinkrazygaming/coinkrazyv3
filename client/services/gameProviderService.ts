...
class GameProviderService {
    ...
    const sportsbookEvents: ProviderGame[] = [
      {
        ...
        volatility: "medium",
        maxWin: 1000000,
        minBet: { GC: 0, SC: 1.0 }, // Sports only accepts SC
  ...
  private generatePragmaticPlayGames(count: number): ProviderGame[] {
    ...
    for (let i = 0; i < count; i++) {
      ...
      games.push({
        ...
        minBet: {
          GC: 10 + Math.floor(Math.random() * 20),
          SC: 0.1 + Math.random() * 0.2,
        },
        maxBet: {
          GC: 5000 + Math.floor(Math.random() * 10000),
          SC: 50 + Math.floor(Math.random() * 100),
        },
  ...
  private generateBetsoftGames(count: number): ProviderGame[] {
    ...
    for (let i = 0; i < count; i++) {
      ...
      games.push({
        ...
        minBet: {
          GC: 25 + Math.floor(Math.random() * 25),
          SC: 0.25 + Math.random() * 0.25,
        },
        maxBet: {
          GC: 7500 + Math.floor(Math.random() * 7500),
          SC: 75 + Math.floor(Math.random() * 75),
        },
        features: [
          "3D Graphics",
          "Interactive Bonus",
  ...
  private generateNetEntGames(count: number): ProviderGame[] {
    ...
    for (let i = 0; i < count; i++) {
      ...
      games.push({
        ...
        minBet: {
          GC: 10 + Math.floor(Math.random() * 15),
          SC: 0.1 + Math.random() * 0.15,
        },
        maxBet: {
          GC: 10000 + Math.floor(Math.random() * 15000),
          SC: 100 + Math.floor(Math.random() * 150),
        },
        features: [
          "Avalanche Reels",
          "Free Spins",
  ...
  private generateProprietaryGames(count: number): ProviderGame[] {
    ...
    for (let i = 0; i < count; i++) {
      ...
      games.push({
        ...
        minBet: {
          GC: 20 + Math.floor(Math.random() * 30),
          SC: 0.2 + Math.random() * 0.3,
        },
        maxBet: { GC: 12500, SC: 125 },
        features: [
          "Social Sharing",
  ...
  private generateEvolutionGames(count: number): ProviderGame[] {
    ...
    for (let i = 0; i < count; i++) {
      ...
      games.push({
        ...
        minBet: {
          GC: 50 + Math.floor(Math.random() * 100),
          SC: 0.5 + Math.random() * 1.0,
        },
        maxBet: {
          GC: 250000 + Math.floor(Math.random() * 250000),
          SC: 2500 + Math.floor(Math.random() * 2500),
        },
        features: [
          "Live Dealer",
          "HD Streaming",
  ...
  async createGameSession(
    ...
    if (!provider.supportedCurrencies.includes(currency)) {
      throw new Error(
        `Currency ${currency} not supported by provider ${provider.displayName}`,
      );
    }
