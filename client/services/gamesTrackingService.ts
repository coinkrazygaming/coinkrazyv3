...
class GamesTrackingService {
  ...
  private updateRealTimeStats() {
    ...
    if (increment > 0.5) {
      this.platformStats.dailyStats.totalWinsToday +=
  ...
  updateGameCount(categoryId: string, newCount: number): boolean {
    const category = this.platformStats.gamesByCategory.find(
  ...
  addNewGame(categoryId: string, providerId?: string) {
    ...
    if (providerId) {
      const provider = this.platformStats.gamesByProvider.find(
  ...
  getTopPerformingGames(): {
    ...
    return [
      { gameId: "coinkrazy-spinner", plays: 1247, scAwarded: 234.5 },
      { gameId: "lucky-scratch-gold", plays: 987, scAwarded: 189.25 },
      { gameId: "sweet-bonanza", plays: 856, scAwarded: 167.8 },
      { gameId: "gates-olympus", plays: 743, scAwarded: 145.6 },
      { gameId: "book-dead", plays: 692, scAwarded: 134.2 },
    ];
  }
