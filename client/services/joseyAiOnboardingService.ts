export interface UserAccount {
  ...
  onboardingStage: 'welcome' | 'education' | 'kyc_required' | 'kyc_pending' | 'kyc_approved' | 'completed';
  kycStatus: 'not_started' | 'documents_required' | 'under_review' | 'approved' | 'rejected';
  flags: {
    newUser: boolean;
    kycRequired: boolean;
...
export interface JoseyAiResponse {
  ...
  timestamp: Date;
  actionRequired?: {
    type: 'upload_document' | 'verify_info' | 'complete_education' | 'contact_support';
    description: string;
    url?: string;
  };
  contextAware: {
...
class JoseyAiOnboardingService {
  ...
  private listeners: Set<(userId: string, response: JoseyAiResponse) => void> = new Set();
    ...
    const userSteps: OnboardingStep[] = [
      {
        ...
        required: true,
        completed: false,
        joseyAiScript: "🎉 Welcome to CoinKrazy! I'm JoseyAI, your personal assistant! I'm here to help you navigate our amazing sweepstakes casino platform. Let's start by showing you what makes us special!",
      ...
      },
      {
        ...
        completed: false,
        dependsOn: ['welcome'],
        joseyAiScript: "Let me explain what makes CoinKrazy different from other sweepstakes platforms. We offer REAL instant withdrawals, better odds, more games, and genuine customer support - not chatbots!",
      ...
      },
      {
        ...
        completed: false,
        dependsOn: ['education'],
        joseyAiScript: "To ensure security and enable withdrawals, we need to verify your identity. This is standard for all legitimate sweepstakes platforms and protects both you and us!",
      },
      {
        id: 'kyc_documents',
        ...
        joseyAiScript: "Please upload a clear photo of your government-issued ID and a recent utility bill. Make sure the names and addresses match exactly. I'm here to help if you have questions!",
      ...
      },
      {
        ...
        joseyAiScript: "🎊 Congratulations! You're now fully verified and ready to enjoy everything CoinKrazy has to offer. Welcome to the family!",
    ...
    const staffSteps: OnboardingStep[] = [
      {
        ...
        required: true,
        completed: false,
        joseyAiScript: "Welcome to the CoinKrazy team! I'm JoseyAI and I'll help you get familiar with our systems, policies, and tools. Let's make sure you're ready to provide excellent customer service!",
      },
      {
        id: 'system_training',
        ...
        joseyAiScript: "Let me walk you through our admin systems, customer support tools, and escalation procedures. You'll also learn about our unique competitive advantages.",
      },
      {
        ...
        joseyAiScript: "Important: Here's our compliance training covering sweepstakes laws, responsible gaming, and data protection. This is mandatory for all staff members.",
      }
    ];
    ...
    const adminSteps: OnboardingStep[] = [
      {
        ...
        required: true,
        completed: false,
        joseyAiScript: "Welcome to full admin access! I'll brief you on all systems, security protocols, and advanced features. You now have complete control over the platform.",
      },
      {
        ...
        joseyAiScript: "Critical: Here are the security protocols, API keys, database access, and emergency procedures. Please review all security measures carefully.",
      }
    ];
  ...
  private sendOnboardingReminders() {
    ...
    this.userAccounts.forEach((account, userId) => {
      if (!account.onboardingRequired || account.onboardingStage === 'completed') return;
  ...
  private sendContextualReminder(userId: string, account: UserAccount) {
    ...
    switch (account.onboardingStage) {
      case 'welcome':
        ...
        incentive = 'Complete your welcome tour now and get 5,000 Gold Coins + 5 Sweeps Coins!';
        break;
      case 'education':
      ...
      case 'kyc_pending':
        ...
        incentive = 'Complete KYC verification and receive 10 Sweeps Coins bonus!';
        break;
    }
  ...
  getOnboardingSteps(accountType: 'user' | 'staff' | 'admin'): OnboardingStep[] {
  ...
  completeOnboardingStep(userId: string, stepId: string): boolean {
    ...
    if (step.incentives) {
      ...
      console.log(`Awarding ${step.incentives.gcReward || 0} GC and ${step.incentives.scReward || 0} SC to ${userId}`);
    }
  ...
  private updateOnboardingStage(userId: string, account: UserAccount) {
    ...
    if (completedSteps.includes('welcome') && !completedSteps.includes('education')) {
      account.onboardingStage = 'education';
    } else if (completedSteps.includes('education') && !completedSteps.includes('kyc_documents')) {
      account.onboardingStage = 'kyc_required';
    } else if (completedSteps.includes('kyc_documents') && account.kycStatus !== 'approved') {
  ...
  private sendNextStepMessage(userId: string, account: UserAccount, completedStepId: string) {
    const steps = this.getOnboardingSteps(account.accountType);
    const nextStep = steps.find(s => 
      !s.completed && 
      (!s.dependsOn || s.dependsOn.every(dep => account.onboardingData.completedSteps.includes(dep)))
    );
    ...
    if (nextStep) {
      const response: JoseyAiResponse = {
        ...
        timestamp: new Date(),
        actionRequired: {
          type: nextStep.type === 'kyc' ? 'upload_document' : 'complete_education',
  ...
  private sendCompletionMessage(userId: string, account: UserAccount) {
    const response: JoseyAiResponse = {
      ...
      userId,
      message: "🎊 Congratulations! You've completed your onboarding! Welcome to the CoinKrazy family. You now have full access to all features, including withdrawals. I'm always here if you need help!",
      type: 'welcome',
      timestamp: new Date(),
      contextAware: {
        userLocation: 'onboarding',
  ...
  updateKycStatus(userId: string, status: UserAccount['kycStatus']): boolean {
    ...
    if (status === 'approved') {
      const response: JoseyAiResponse = {
        ...
        userId,
        message: "🎉 Great news! Your KYC verification has been approved! You can now make withdrawals and access all premium features. Welcome to full CoinKrazy membership!",
        type: 'welcome',
        timestamp: new Date(),
        contextAware: {
          userLocation: 'dashboard',
  ...
  subscribeToResponses(callback: (userId: string, response: JoseyAiResponse) => void): () => void {
    this.listeners.add(callback);
    return () => this.listeners.delete(callback);
  }

  // Context awareness methods
  updateUserContext(userId: string, context: Partial<JoseyAiResponse['contextAware']>) {
    ...
    if (context.currentPage === 'games' && account.onboardingStage !== 'completed') {
    ...
    switch (pageType) {
      case 'games':
        if (account.kycStatus !== 'approved') {
          message = "I see you're checking out our games! Just a friendly reminder that completing your KYC verification will unlock withdrawal capabilities for any SC you win. Want me to help you finish that process?";
        }
        break;
    }
  ...
  getAllPendingUsers(): UserAccount[] {
    return Array.from(this.userAccounts.values())
      ...
      .sort((a, b) => b.onboardingData.startDate.getTime() - a.onboardingData.startDate.getTime());
  }
