...
class LuckyAIService {
    ...
    setInterval(() => {
      this.vmInstances.forEach((vm, id) => {
        ...
        if (vm.status === "running") {
          vm.cpu = Math.max(
            10,
            Math.min(95, vm.cpu + (Math.random() - 0.5) * 10),
          );
          vm.memory = Math.max(
            15,
            Math.min(90, vm.memory + (Math.random() - 0.5) * 8),
          );
          vm.disk = Math.max(
            10,
            Math.min(95, vm.disk + (Math.random() - 0.5) * 2),
          );
  ...
  private generateVMLog(vm: VMInstance): string {
    ...
    const templates = logTemplates[vm.id as keyof typeof logTemplates] || [
    ...
    this.chatHistory = [
      {
        ...
        role: "system",
        content:
          "LuckyAI initialized successfully. All systems operational. VM monitoring active.",
      ...
      },
      {
        ...
        role: "assistant",
        content:
          "Hello! I'm Lucky, your AI assistant. I'm here to help manage the casino operations, monitor system health, and assist with any questions you might have. How can I help you today?",
...
${runningVMs
  .map(
    (vm) => `• ${vm.name}: CPU ${vm.cpu}% | RAM ${vm.memory}% | ${vm.purpose}`,
  )
  .join("\n")}
...
${sortedByCPU
  .slice(0, 2)
  .map((vm) => `• ${vm.name}: ${vm.cpu}% CPU, ${vm.memory}% RAM`)
  .join("\n")}
...
Immediate action recommended: Check database connection and service logs.`;
  ...
  private async updateAIMetrics(): Promise<void> {
    try {
      await databaseService.updateAIEmployeeMetrics(1, 1, 5.5); // LuckyAI completed 1 task, saved $5.50
    } catch (error) {
  ...
  getPerformanceMetrics() {
    ...
    const runningVMs = Array.from(this.vmInstances.values()).filter(
      (vm) => vm.status === "running",
    ).length;
    const avgCPU =
      Array.from(this.vmInstances.values()).reduce(
        (sum, vm) => sum + vm.cpu,
        0,
      ) / totalVMs;
    const avgMemory =
    ...
    return {
      ...
      capabilities: {
        ...
        avgPerformance:
          this.capabilities.reduce((sum, c) => sum + c.performance, 0) /
