...
class NeonDatabaseService {
  ...
  private saveLocalData() {
    try {
      ...
      localStorage.setItem('neon_payment_logs', JSON.stringify(this.paymentLogs));
    } catch (error) {
  ...
  async logAdminAction(
    ...
    try {
      if (this.isConnected && this.connectionPool) {
        ...
        console.log(`📋 Admin Log [${severity.toUpperCase()}]: ${action} by ${adminUserId}`);
        this.connectionPool.totalQueries++;
        this.connectionPool.lastQuery = new Date();
      }
  ...
  async logPayment(paymentLog: PaymentLog): Promise<void> {
    ...
    try {
      if (this.isConnected && this.connectionPool) {
        ...
        console.log('💳 Payment Log:', paymentLog.paymentMethod, paymentLog.amount);
        this.connectionPool.totalQueries++;
        this.connectionPool.lastQuery = new Date();
      }
  ...
  async getAdminLogs(
    ...
    if (adminUserId) {
      filteredLogs = filteredLogs.filter(log => log.adminUserId === adminUserId);
    }
  ...
  async getPaymentLogs(
    ...
    if (paymentMethod) {
      filteredLogs = filteredLogs.filter(log => log.paymentMethod === paymentMethod);
    }
  ...
  async getDatabaseStats(): Promise<DatabaseStats> {
    ...
    const last24HourPayments = this.paymentLogs.filter(log => 
      log.timestamp >= oneDayAgo && log.status === 'completed'
    );
    ...
    return {
      ...
      totalDeposits: this.paymentLogs.filter(log => log.status === 'completed').length,
      totalDepositAmount: this.paymentLogs.reduce((sum, log) => 
        log.status === 'completed' ? sum + log.amount : sum, 0
      ),
      totalGCIssued: this.paymentLogs.reduce((sum, log) => 
        log.status === 'completed' ? sum + log.gcAwarded : sum, 0
      ),
      totalSCIssued: this.paymentLogs.reduce((sum, log) => 
        log.status === 'completed' ? sum + log.scAwarded : sum, 0
      ),
      last24HourDeposits: last24HourPayments.length,
      last24HourAmount: last24HourPayments.reduce((sum, log) => sum + log.amount, 0),
      topPaymentMethods
    };
  }
  ...
  async clearOldLogs(daysToKeep: number = 30): Promise<void> {
    ...
    this.adminLogs = this.adminLogs.filter(log => log.timestamp >= cutoffDate);
    this.paymentLogs = this.paymentLogs.filter(log => log.timestamp >= cutoffDate);
  ...
  async exportLogs(format: 'json' | 'csv' = 'json'): Promise<string> {
    ...
    if (format === 'json') {
      ...
      this.adminLogs.forEach(log => {
        csvLines.push(`Admin,${log.timestamp.toISOString()},${log.adminUserId},${log.action},${log.targetUserId || ''},${JSON.stringify(log.details).replace(/"/g, '""')}`);
      });

      this.paymentLogs.forEach(log => {
        csvLines.push(`Payment,${log.timestamp.toISOString()},${log.userId},${log.paymentMethod},${log.amount},$${log.amount}`);
      });
