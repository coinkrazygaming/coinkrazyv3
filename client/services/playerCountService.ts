...
class PlayerCountService {
  ...
  private listeners: Set<(count: number, stats: PlayerStats) => void> = new Set();
  ...
  private initializeSimulatedPlayers() {
    ...
    initialPlayers.forEach((player, index) => {
      const sessionStart = new Date(now.getTime() - Math.random() * 120 * 60 * 1000); // 0-120 minutes ago
      ...
      this.onlinePlayers.set(player.id, {
        ...
        sessionDuration: Math.floor((now.getTime() - sessionStart.getTime()) / (1000 * 60))
      });

      this.sessionStartTimes.set(player.id, sessionStart);
    });

    this.peakTodayCount = this.onlinePlayers.size + Math.floor(Math.random() * 20); // Peak was higher
  ...
  private simulatePlayerJoin() {
    ...
    const usernames = [
      'SpinWinner', 'LuckyStrike', 'JackpotHunter', 'GoldSeeker', 'SlotFan',
      'BingoStar', 'PokerAce', 'RouletteKing', 'CasinoChamp', 'MegaWin',
      'DiamondPlayer', 'VegasVibes', 'FortuneSeeker', 'WildSpinner', 'BonusHunter'
    ];
    
    const games = [
      'Sweet Bonanza', 'Gates of Olympus', 'Book of Dead', 'Starburst',
      'Gonzo\'s Quest', 'Bonanza', 'Reactoonz', 'Big Bad Wolf',
      'Bingo 75-Ball', 'Bingo 90-Ball', 'Texas Holdem', 'Blackjack',
      'European Roulette', 'American Roulette', 'Baccarat'
    ];

    const username = usernames[Math.floor(Math.random() * usernames.length)] + Math.floor(Math.random() * 999);
    const currentGame = Math.random() > 0.3 ? games[Math.floor(Math.random() * games.length)] : undefined;
  ...
  private simulatePlayerLeave() {
    ...
    const removableIds = playerIds.filter(id => 
      !id.includes('coinkrazy') && 
      !id.includes('player-00')
    );
    
    if (removableIds.length === 0) return;

    const randomId = removableIds[Math.floor(Math.random() * removableIds.length)];
  ...
  private simulateActivityUpdate() {
    ...
    for (let i = 0; i < updateCount; i++) {
      ...
      if (player) {
        ...
        if (sessionStart) {
          player.sessionDuration = Math.floor((Date.now() - sessionStart.getTime()) / (1000 * 60));
        }
      }
    }
  ...
  getOnlinePlayers(): PlayerActivity[] {
    return Array.from(this.onlinePlayers.values()).sort((a, b) => 
  ...
  getPlayerStats(): PlayerStats {
    ...
    const totalSessionTime = players.reduce((sum, player) => sum + player.sessionDuration, 0);
    const averageSessionTime = players.length > 0 ? totalSessionTime / players.length : 0;
  ...
  subscribeToPlayerCount(callback: (count: number, stats: PlayerStats) => void): () => void {
