...
class ProductionService {
  ...
  private initializeProductionConfig(): ProductionConfig {
    const environment = (import.meta.env.VITE_ENVIRONMENT || "production") as
      | "production"
      | "staging"
      | "development";
    ...
    return {
      ...
      apiEndpoints: {
        ...
        payments:
          import.meta.env.VITE_PAYMENTS_API ||
          "https://api.coinkrazy.com/payments",
        games:
          import.meta.env.VITE_GAMES_API || "https://api.coinkrazy.com/games",
        sports:
          import.meta.env.VITE_SPORTS_API || "https://api.coinkrazy.com/sports",
        support:
          import.meta.env.VITE_SUPPORT_API ||
          "https://api.coinkrazy.com/support",
        analytics:
          import.meta.env.VITE_ANALYTICS_API ||
          "https://api.coinkrazy.com/analytics",
      },
      security: {
  ...
  private updateSystemHealth() {
    ...
    const offlineServices = Object.values(this.systemHealth.services).filter(
      (s) => s.status === "offline",
    ).length;
    const degradedServices = Object.values(this.systemHealth.services).filter(
      (s) => s.status === "degraded",
    ).length;
    ...
    this.systemHealth.metrics = {
      cpuUsage: Math.max(
        ...
        Math.min(
          90,
          this.systemHealth.metrics.cpuUsage + (Math.random() - 0.5) * 5,
        ),
      ),
      memoryUsage: Math.max(
        ...
        Math.min(
          95,
          this.systemHealth.metrics.memoryUsage + (Math.random() - 0.5) * 3,
        ),
      ),
      diskUsage: Math.max(
        20,
        Math.min(
          85,
          this.systemHealth.metrics.diskUsage + (Math.random() - 0.5) * 1,
        ),
      ),
      activeUsers: Math.max(
        100,
        this.systemHealth.metrics.activeUsers +
          Math.floor((Math.random() - 0.5) * 20),
      ),
      requestsPerMinute: Math.max(
        1000,
        this.systemHealth.metrics.requestsPerMinute +
          Math.floor((Math.random() - 0.5) * 200),
      ),
      errorRate: Math.max(
        0,
        Math.min(
          0.1,
          this.systemHealth.metrics.errorRate + (Math.random() - 0.5) * 0.01,
        ),
      ),
    };
  ...
  private checkForAlerts() {
    ...
    this.systemHealth.alerts = this.systemHealth.alerts.filter(
  ...
  private generateSystemReport() {
    const report = {
      ...
      securitySummary: {
        ...
        suspiciousActivity: this.securityEvents.filter(
  ...
  getPerformanceMetrics(): PerformanceMetric[] {
    return this.performanceMetrics.sort(
      (a, b) => b.timestamp.getTime() - a.timestamp.getTime(),
    );
  }
      ...
      if (!isHealthy) {
        this.recordSecurityEvent({
          ...
          details: {
            ...
            unhealthyServices: criticalServices.filter(
              (s) => !healthyServices.includes(s),
            ),
          },
          severity: "high",
        });
