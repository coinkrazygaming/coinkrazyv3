...
class SlotGameEngine {
  ...
  private createCoinKrazyBonusFeatures(): BonusFeature[] {
    return [
      ...
      },
      {
        ...
        rewards: [{ type: "multiplier", value: 2, applies_to: "next_spin" }],
        isActive: true,
      },
    ];
  }
    ...
    return [
      {
        ...
        rewards: [{ type: "free_spins", value: 1, applies_to: "next_spin" }],
        isActive: true,
      },
      {
        id: "ante-bet",
        ...
        rewards: [{ type: "multiplier", value: 2, applies_to: "session" }],
        isActive: true,
      },
      {
        id: "free-spins-sweet",
    ...
    return [
      {
        ...
        rewards: [{ type: "multiplier", value: 500, applies_to: "next_spin" }],
        isActive: true,
      },
      {
    ...
    return [
      {
        ...
        rewards: [{ type: "jackpot_trigger", value: 1 }],
        isActive: true,
      },
    ];
  }
        ...
        config.jackpots?.forEach((jackpot) => {
          if (jackpot.type === "progressive") {
            ...
            localStorage.setItem(
              `jackpot_${jackpot.id}`,
              jackpot.currentAmount.toString(),
            );
          }
        });
      });
  ...
  async spin(
    ...
    // Validate bet
    if (
      betAmount < config.minBet[currency] ||
      betAmount > config.maxBet[currency]
    ) {
      throw new Error(
        `Invalid bet amount. Must be between ${config.minBet[currency]} and ${config.maxBet[currency]} ${currency}`,
      );
    }
  ...
  private generateSpinResult(
    ...
    if (
      ...
      // Cluster pays system
      const clusterWins = this.calculateClusterWins(
        reels,
        config.symbols,
        betAmount,
      );
      ...
      const paylineWins = this.calculatePaylineWins(
        reels,
        config,
        betAmount,
        selectedPaylines,
    ...
    return {
      ...
      paylines,
      totalWin,
      bonusTriggered: bonusTriggered.triggered
        ? bonusTriggered.feature
        : undefined,
      jackpotWin,
  ...
  private calculatePaylineWins(
    ...
    const activePaylines =
      selectedPaylines ||
      config.paylines.filter((p) => p.isActive).map((p) => p.id);
    ...
    activePaylines.forEach((paylineId) => {
      ...
      const win = this.calculatePaylineWin(
        paylineSymbols,
        config.symbols,
        betAmount,
        paylineId,
      ...
      for (let row = 0; row < reels[reel].length; row++) {
        if (!visited[reel][row]) {
          ...
          if (cluster.length >= 8) {
            ...
            const symbolData = gameSymbols.find(
              (s) => s.id === reels[reel][row],
            );
            if (symbolData) {
  ...
  private isBonusTriggered(
    ...
    switch (condition.type) {
      case "symbol_combination":
        return (
          this.countSymbolOccurrences(reels, condition.symbols![0]) >=
          (condition.count || 3)
        );
  ...
  private isJackpotTriggered(
    ...
    if (condition.type === "random") {
      ...
      const adjustedProbability =
        (condition.probability || 0.0001) * Math.min(betAmount / 100, 10);
    ...
    while (true) {
      const wins =
        config.theme === "candy"
          ...
          : {
              wins: this.calculatePaylineWins(currentReels, config, betAmount),
              totalWin: 0,
            };
      ...
      const symbolsAdded = this.fillReelsAfterCascade(
        currentReels,
        config.symbols,
      );
  ...
  private updateSession(
    ...
    if (session) {
      ...
      session.rtp =
        session.totalBet > 0 ? (session.totalWin / session.totalBet) * 100 : 0;
  ...
  private getMostTriggeredFeatures(sessions: GameSession[]): {
    [key: string]: number;
  } {
