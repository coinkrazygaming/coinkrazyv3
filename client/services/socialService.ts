...
class SocialService {
  ...
  private activityListeners: Set<(activity: SocialActivity) => void> =
    new Set();
  private notificationListeners: Set<
    (notification: SocialNotification) => void
  > = new Set();
  ...
  private initializeWebSocket() {
    ...
    try {
      ...
      this.ws.onclose = (event) => {
        console.log(
          `Social WebSocket disconnected: Code ${event.code}, Reason: ${event.reason}`,
        );
  ...
  async getUserProfile(userId: string): Promise<SocialUser | null> {
    try {
      ...
      if (!response.ok) {
        console.warn(
          `User profile API returned ${response.status}, using mock data`,
        );
  ...
  async getSocialFeed(
    ...
    try {
      const response = await this.fetchWithTimeout(
        `${this.baseUrl}/feed?page=${page}&limit=${limit}`,
      );
  ...
  async joinChallenge(challengeId: string): Promise<boolean> {
    try {
      const response = await this.fetchWithTimeout(
        `${this.baseUrl}/challenges/${challengeId}/join`,
        {
          method: "POST",
        },
      );
  ...
  async getLeaderboard(
    type: "wins" | "earnings" | "level" | "social" = "wins",
  ): Promise<SocialUser[]> {
    try {
      const response = await this.fetchWithTimeout(
        `${this.baseUrl}/leaderboard/${type}`,
      );
  ...
  async getUserBadges(userId?: string): Promise<SocialBadge[]> {
    try {
      const response = await this.fetchWithTimeout(
        `${this.baseUrl}/badges${userId ? `/${userId}` : ""}`,
      );
  ...
  onNotification(
    listener: (notification: SocialNotification) => void,
  ): () => void {
    ...
    return [
      {
        ...
        content:
          "Just hit a massive win on Sweet Bonanza! 🍭💰 The multipliers were insane!",
        images: [],
        type: "win",
        metadata: {
          gameId: "sweet-bonanza",
      ...
      },
      {
        ...
        content:
          "Finally reached level 30! 🎉 Thanks to everyone who helped along the way. The grind was real but so worth it!",
        images: [],
        type: "milestone",
        metadata: {
      ...
      },
      {
        ...
        content:
          "Who's ready for the weekend tournament? 🏆 The prize pool is looking amazing! Let's see who can claim the crown this time 👑",
        images: [],
        type: "tournament",
        metadata: {
    ...
    return [
      {
        ...
        status: "pending",
        message:
          "Hey! I saw your big win posts. Would love to be friends and share tips!",
        createdAt: new Date().toISOString(),
      },
    ];
  }
  ...
  private getMockGroups(): SocialGroup[] {
    return [
      ...
      },
      {
        ...
        description:
          "All about slots! Share your wins, strategies, and favorite games.",
        ...
        leaderboard: [
          {
            ...
            avatar:
              "https://api.dicebear.com/7.x/avataaars/svg?seed=goldhunter",
            score: 35000,
            rank: 1,
          },
        ],
  ...
  private getMockMessages(): DirectMessage[] {
    ...
    return [
      ...
      },
      {
        ...
        content:
          "Absolutely! I was thinking the same thing. Let's practice together first.",
        type: "text",
        isRead: true,
    ...
    return [
      {
        ...
        stats: {
          ...this.getMockUser("leader1").stats,
          rank: 1,
          totalWins: 500,
        },
      },
      {
        ...
        stats: {
          ...this.getMockUser("leader2").stats,
          rank: 2,
          totalWins: 450,
        },
      },
      {
        ...
        stats: {
          ...this.getMockUser("leader3").stats,
          rank: 3,
          totalWins: 400,
        },
      },
