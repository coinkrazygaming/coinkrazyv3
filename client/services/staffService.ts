...
export interface SupportTicket {
  ...
  subject: string;
  description: string;
  category: 'account' | 'payment' | 'technical' | 'kyc' | 'general' | 'complaint' | 'bonus';
...
export interface UserAccount {
  ...
  kycDocuments: {
    ...
    additionalDocs?: Array<{ type: string; url: string; verified: boolean; uploadedAt: Date }>;
  };
...
export interface StaffAlert {
  id: string;
  type: 'urgent_ticket' | 'escalation' | 'system_issue' | 'performance_alert' | 'compliance_issue';
...
class StaffService {
  ...
  private initializeMockData() {
    ...
    const mockTickets: SupportTicket[] = [
      {
        ...
        description: 'I requested a withdrawal 3 days ago but haven\'t received it yet. This is my first withdrawal and I\'m concerned.',
        category: 'payment',
        priority: 'high',
        status: 'open',
        ...
        messages: [
          {
            ...
            senderId: 'user_123',
            senderType: 'user',
            content: 'I requested a withdrawal 3 days ago but haven\'t received it yet. This is my first withdrawal and I\'m concerned.',
            isInternal: false,
            attachments: [],
      ...
      },
      {
        ...
        description: 'The CoinKrazy Spinner game won\'t load on my mobile device. It worked fine yesterday.',
        category: 'technical',
        priority: 'medium',
        status: 'in_progress',
        ...
        messages: [
          {
            ...
            senderId: 'user_456',
            senderType: 'user',
            content: 'The CoinKrazy Spinner game won\'t load on my mobile device. It worked fine yesterday.',
            isInternal: false,
            attachments: [],
          ...
          },
          {
            ...
            senderId: 'staff_002',
            senderType: 'staff',
            content: 'Hi! I\'m sorry to hear you\'re having trouble with the game. Can you please try clearing your browser cache and cookies, then try again?',
            isInternal: false,
            attachments: [],
        ...
        internalNotes: ['Common mobile loading issue - standard troubleshooting applied'],
        escalated: false,
        aiAnalysis: {
          sentiment: 'neutral',
      ...
      },
      {
        ...
        description: 'I reached VIP level but don\'t see the promised benefits. Can you explain what I should expect?',
        category: 'account',
        priority: 'medium',
        status: 'waiting_user',
        assignedTo: 'staff_001',
        ...
        messages: [
          {
            ...
            senderId: 'user_789',
            senderType: 'user',
            content: 'I reached VIP level but don\'t see the promised benefits. Can you explain what I should expect?',
            isInternal: false,
            attachments: [],
          ...
          },
          {
            ...
            senderId: 'staff_001',
            senderType: 'staff',
            content: 'Congratulations on reaching VIP status! Your benefits include: faster withdrawals, dedicated support, exclusive bonuses, and higher limits. These should be active now - can you check your account?',
            isInternal: false,
            attachments: [],
        ...
        internalNotes: ['VIP user - handle with priority, ensure all benefits are properly applied'],
        escalated: false,
        aiAnalysis: {
          sentiment: 'positive',
    ...
    const mockUsers: UserAccount[] = [
      {
        ...
        kycStatus: 'approved',
        kycDocuments: {
          photoId: { url: '/docs/id_123.jpg', verified: true, uploadedAt: new Date('2024-03-15T10:00:00') },
          addressProof: { url: '/docs/address_123.pdf', verified: true, uploadedAt: new Date('2024-03-15T10:30:00') }
        },
      ...
      },
      {
        ...
        kycStatus: 'pending',
        kycDocuments: {
          photoId: { url: '/docs/id_456.jpg', verified: false, uploadedAt: new Date('2024-03-18T14:00:00') }
        },
    ...
    const mockAlerts: StaffAlert[] = [
      ...
      },
      {
        ...
        metadata: { escalationType: 'technical', originalAssignee: 'staff_002' }
      },
      {
        id: 'alert_003',
  ...
  private checkResponseTimes() {
    ...
    this.tickets.forEach(ticket => {
      if (ticket.status === 'open' && !ticket.firstResponseAt) {
        ...
        if (ticket.priority === 'urgent' && timeSinceCreated > urgentThreshold) {
          shouldAlert = true;
          alertPriority = 'critical';
        } else if (ticket.priority === 'high' && timeSinceCreated > highThreshold) {
          shouldAlert = true;
          alertPriority = 'high';
        } else if (ticket.priority === 'medium' && timeSinceCreated > mediumThreshold) {
          shouldAlert = true;
  ...
  private generatePerformanceAlerts() {
    ...
    if (recentTickets.length > 0) {
      const avgResponseTime = recentTickets.reduce((sum, ticket) => {
  ...
  private updateTicketAnalysis() {
    ...
    this.tickets.forEach(ticket => {
      ...
      const hasNegativeKeywords = latestMessages.some(msg => 
      ...
      if ((messageCount > 5 || hoursOpen > 24) && !ticket.escalated && ticket.priority !== 'low') {
  ...
  private cleanupOldAlerts() {
    ...
    this.alerts = this.alerts.filter(alert => 
      alert.createdAt > oneDayAgo || !alert.resolvedAt
    );

    if (this.alerts.length < initialCount) {
      this.notifyListeners('alerts_cleaned', { removed: initialCount - this.alerts.length });
    }
  }
  ...
  getTicketsByAssignee(staffId: string): SupportTicket[] {
    return Array.from(this.tickets.values()).filter(ticket => ticket.assignedTo === staffId);
  }
  ...
  updateTicketStatus(ticketId: string, status: SupportTicket['status'], staffId: string): boolean {
    ...
    if (status === 'resolved') {
      ...
      ticket.resolutionTime = Math.round(
        (ticket.resolvedAt.getTime() - ticket.createdAt.getTime()) / (60 * 1000)
      );
    }
  ...
  escalateTicket(ticketId: string, reason: string, staffId: string): boolean {
    ...
    this.alerts.push(alert);
    this.notifyListeners('ticket_escalated', { ticket, reason, staffId, alert });
    return true;
  }
  ...
  getUsersByKycStatus(status: UserAccount['kycStatus']): UserAccount[] {
    return Array.from(this.users.values()).filter(user => user.kycStatus === status);
  }

  updateUserStatus(userId: string, status: UserAccount['status'], reason: string, staffId: string): boolean {
    const user = this.users.get(userId);
    if (!user) return false;

    const previousStatus = user.status;
    user.status = status;
    user.notes.push(`Status changed from ${previousStatus} to ${status} by ${staffId}: ${reason}`);

    this.notifyListeners('user_status_updated', { user, previousStatus, reason, staffId });
    return true;
  }

  updateKycStatus(userId: string, status: UserAccount['kycStatus'], notes: string, staffId: string): boolean {
    const user = this.users.get(userId);
    if (!user) return false;

    const previousStatus = user.kycStatus;
    user.kycStatus = status;
    user.notes.push(`KYC status changed from ${previousStatus} to ${status} by ${staffId}: ${notes}`);

    // If approved, remove withdrawal restrictions
    if (status === 'approved' && user.restrictions.includes('withdrawal_disabled')) {
      user.restrictions = user.restrictions.filter(r => r !== 'withdrawal_disabled');
    }

    // Send notification email
    emailService.sendKycStatusUpdate(user.email, status, notes);

    this.notifyListeners('kyc_status_updated', { user, previousStatus, notes, staffId });
    return true;
  }
  ...
  updateStaffPerformance(staffId: string, metrics: Partial<StaffPerformance>): boolean {
  ...
  getTicketAnalytics(period: { start: Date; end: Date }) {
    const tickets = Array.from(this.tickets.values()).filter(
      ticket => ticket.createdAt >= period.start && ticket.createdAt <= period.end
    );
  ...
  private generateDailyVolume(tickets: SupportTicket[], period: { start: Date; end: Date }) {
  ...
  async sendTicketUpdateEmail(ticketId: string, updateType: string): Promise<boolean> {
    const ticket = this.tickets.get(ticketId);
    if (!ticket) return false;
  ...
  async exportUserData(userId: string): Promise<any> {
    ...
    const userTickets = Array.from(this.tickets.values()).filter(t => t.userId === userId);
