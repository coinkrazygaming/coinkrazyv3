...
export type RouletteBetType = 
  ...
  | 'red' | 'black' | 'odd' | 'even' | 'low' | 'high' | 'zero' | 'double_zero';
...
export interface BaccaratBet {
  type: 'player' | 'banker' | 'tie' | 'player_pair' | 'banker_pair' | 'perfect_pair';
  amount: number;
  odds: number;
}
...
class TableGameEngine {
  ...
  createStandardDeck(): PlayingCard[] {
    ...
    suits.forEach(suit => {
      ranks.forEach(rank => {
        let value = 0;
        if (rank === 'A') value = 11; // Ace high initially
  ...
  private getCardUnicode(suit: PlayingCard['suit'], rank: PlayingCard['rank']): string {
    const suitSymbols = {
      hearts: '♥',
      diamonds: '♦',
      clubs: '♣',
  ...
  async placeBet(gameId: string, betAmount: number, sideBets?: { [key: string]: number }): Promise<boolean> {
    ...
    if (betAmount < game.minBet || betAmount > game.maxBet) {
      throw new Error(`Bet must be between ${game.minBet} and ${game.maxBet} ${game.currency}`);
    }

    // Deduct bet from wallet
    await walletService.placeBet(game.userId, betAmount, game.currency, gameId, 'table');
  ...
  private async dealInitialCards(game: BlackjackGameState): Promise<void> {
    ...
    if (playerHand.value === 21) {
      ...
      if (game.dealerHand.cards[0].value === 10 || game.dealerHand.cards[0].rank === 'A') {
  ...
  private calculateHandValue(hand: BlackjackHand, dealerFirstCardOnly = false): number {
    let value = 0;
    let aces = 0;
  ...
  private async playerHit(game: BlackjackGameState, hand: BlackjackHand): Promise<void> {
  ...
  private async playerStand(game: BlackjackGameState, hand: BlackjackHand): Promise<void> {
    hand.isStood = true;
    await this.moveToNextHand(game);
  }

  private async playerDouble(game: BlackjackGameState, hand: BlackjackHand): Promise<void> {
    if (!hand.canDouble) {
      throw new Error('Cannot double this hand');
    }
    
    // Place additional bet
    await walletService.placeBet(game.userId, hand.bet, game.currency, game.gameId, 'table');
    hand.bet *= 2;
  ...
  private async playerSplit(game: BlackjackGameState, hand: BlackjackHand): Promise<void> {
  ...
  private async playerSurrender(game: BlackjackGameState, hand: BlackjackHand): Promise<void> {
    if (!game.rules.surrenderAllowed || hand.cards.length !== 2) {
      throw new Error('Cannot surrender this hand');
    }
    
    // Return half the bet
    await walletService.recordWin(game.userId, hand.bet / 2, game.currency, game.gameId, 'table');
  ...
  private async playerInsurance(game: BlackjackGameState, amount: number): Promise<void> {
    if (amount > game.playerHands[0].bet / 2) {
      throw new Error('Insurance cannot exceed half the original bet');
    }
    
    await walletService.placeBet(game.userId, amount, game.currency, game.gameId, 'table');
    ...
    if (dealerValue === 21) {
      // Insurance pays 2:1
      await walletService.recordWin(game.userId, amount * 3, game.currency, game.gameId, 'table');
    }
  }
  ...
  private async playDealerHand(game: BlackjackGameState): Promise<void> {
    ...
    const playableHands = game.playerHands.filter(h => !h.isBusted && !game.surrender);
  ...
  private shouldDealerHit(dealerHand: BlackjackHand, rules: BlackjackRules): boolean {
    if (dealerHand.value < 17) return true;
    if (dealerHand.value > 17) return false;
    
    // Exactly 17
    if (dealerHand.value === 17 && dealerHand.softAce && !rules.dealerStandsOnSoft17) {
  ...
  private async finishHand(game: BlackjackGameState): Promise<void> {
    ...
    for (const playerHand of game.playerHands) {
      const result = this.calculateBlackjackResult(playerHand, game.dealerHand, game.rules);
      totalWin += result.payout;
      
      if (result.payout > 0) {
        await walletService.recordWin(game.userId, result.payout, game.currency, game.gameId, 'table');
      }
    }
  ...
  private calculateBlackjackResult(
    ...
    if (playerHand.isBlackjack) {
      if (dealerHand.isBlackjack) {
        return { result: 'push', payout: playerHand.bet, description: 'Both blackjack - push' };
      }
      return { 
        result: 'win', 
    ...
    if (dealerHand.isBusted) {
      return { result: 'win', payout: playerHand.bet * 2, description: 'Dealer busted' };
    }
    
    // Compare values
    if (playerHand.value > dealerHand.value) {
      return { result: 'win', payout: playerHand.bet * 2, description: 'Player wins' };
  ...
  async startRouletteGame(userId: string, currency: CurrencyType): Promise<RouletteGameState> {
  ...
  async placeRouletteBet(
    ...
    // Deduct bet from wallet
    await walletService.placeBet(game.userId, amount, game.currency, gameId, 'table');
  ...
  private getBetDescription(betType: RouletteBetType, numbers: number[]): string {
    switch (betType) {
      case 'straight': return `Straight ${numbers[0]}`;
      case 'split': return `Split ${numbers.join('/')}`;
      case 'street': return `Street ${numbers.join(',')}`;
      case 'corner': return `Corner ${numbers.join(',')}`;
      case 'line': return `Line ${numbers.join(',')}`;
      case 'column': return `Column ${Math.ceil(numbers[0] / 12)}`;
      case 'dozen': 
  ...
  async spinRoulette(gameId: string): Promise<RouletteResult> {
    ...
    for (const bet of game.bets) {
      if (this.isBetWinning(bet, winningNumber)) {
        ...
        await walletService.recordWin(game.userId, payout, game.currency, gameId, 'table');
      }
    }
  ...
  private generateBallPath(finalNumber: number): number[] {
    ...
    const finalPosition = wheel.find(n => n.number === finalNumber)?.position || 0;
  ...
  // BACCARAT ENGINE
  async startBaccaratGame(userId: string, currency: CurrencyType): Promise<BaccaratGameState> {
  ...
  async dealBaccarat(gameId: string): Promise<BaccaratResult> {
    ...
    if (!playerNatural && !bankerNatural) {
      ...
      const playerThirdCard = game.playerHand.length === 3 ? game.playerHand[2] : null;
    ...
    for (const bet of game.bets) {
      ...
      if (bet.type === winner) {
        ...
        payout = bet.amount + (bet.amount * bet.odds);
      } else if (bet.type === 'perfect_pair' && this.isPerfectPair(game.playerHand, game.bankerHand)) {
        payout = bet.amount + (bet.amount * bet.odds);
      }
  ...
  private shouldBankerDraw(bankerValue: number, playerThirdCard: PlayingCard | null): boolean {
  ...
  private isPerfectPair(playerHand: PlayingCard[], bankerHand: PlayingCard[]): boolean {
  ...
  private updateGameSession(
    ...
    if (winAmount > 0) {
      ...
      session.currentStreak = session.currentStreak < 0 ? session.currentStreak - 1 : -1;
      session.longestLoseStreak = Math.max(session.longestLoseStreak, Math.abs(session.currentStreak));
    }

    session.averageBet = session.totalBets / session.handsPlayed;
    session.houseEdge = session.totalBets > 0 ? ((session.totalBets - session.totalWins) / session.totalBets) * 100 : 0;
  }
  ...
  private recordDecision(game: any, action: string, hand: any): void {
    ...
    const session = Array.from(this.activeSessions.values()).find(
      s => s.userId === game.userId && s.gameType === 'blackjack' && !s.endTime
    );
  ...
  private describeSituation(playerHand: BlackjackHand, dealerHand: BlackjackHand): string {
  ...
  private isOptimalDecision(playerHand: BlackjackHand, dealerHand: BlackjackHand, action: string): boolean {
  ...
  getGameHistory(userId: string, gameType?: string): any[] {
    ...
    return gameType ? userHistory.filter(game => game.gameType === gameType) : userHistory;
  }

  getGameStatistics(gameType: 'blackjack' | 'roulette' | 'baccarat'): any {
    const sessions = Array.from(this.activeSessions.values()).filter(s => s.gameType === gameType);
