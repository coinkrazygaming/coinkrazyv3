...
class TableGamesService {
  ...
  private initializeWebSocket() {
    ...
    try {
      ...
      console.warn('Table Games WebSocket initialization failed, using mock data:', error);
      this.loadMockData();
      this.simulateRealTimeUpdates();
    }
  }
  ...
  private setupSocketListeners() {
    ...
    this.socket.on('roulette_spin', (data: { tableId: string; number: number }) => {
    ...
    this.socket.on('dice_rolled', (data: { tableId: string; dice: [number, number] }) => {
      ...
      if (table) {
        ...
        const roll: CrapsRoll = {
          ...
          isHard: data.dice[0] === data.dice[1] && [4, 6, 8, 10].includes(data.dice[0] + data.dice[1])
        };
        table.lastRoll = roll;
  ...
  private loadMockData() {
    ...
    mockBlackjackTables.forEach(table => this.blackjackTables.set(table.id, table));
    ...
    mockRouletteTables.forEach(table => this.rouletteTables.set(table.id, table));
    ...
    mockBaccaratTables.forEach(table => this.baccaratTables.set(table.id, table));
    ...
    const mockCrapsTables: CrapsTable[] = [
      {
        ...
        maxBet: 5000,
        gamePhase: 'come-out',
        lastRoll: { dice: [4, 3], total: 7, timestamp: new Date(), isNatural: true },
      ...
      },
      {
        ...
        gamePhase: 'point',
        point: 6,
        lastRoll: { dice: [3, 3], total: 6, timestamp: new Date(), isHard: true },
  ...
  private generateBlackjackPlayers(count: number): BlackjackPlayer[] {
    ...
    const names = ['CardCounter', 'BasicStrategy', 'LuckyPlayer', 'HighRoller', 'Conservative', 'Aggressive', 'Newbie'];
  ...
  private generateRouletteePlayers(count: number): RoulettePlayer[] {
    ...
    const names = ['RedPlayer', 'BlackBetter', 'NumberPicker', 'SystemPlayer', 'LuckyBetter', 'Martingale', 'Conservative'];
  ...
  private generateBaccaratPlayers(count: number): BaccaratPlayer[] {
    ...
    const names = ['BankerBetter', 'PlayerPicker', 'TieHunter', 'PatternPlayer', 'HighStakes', 'Conservative'];
  ...
  private generateCrapsPlayers(count: number): CrapsPlayer[] {
    ...
    const names = ['DiceShooter', 'PassLineBetter', 'DontPlayer', 'FieldBetter', 'OddsBetter', 'HardWayPlayer'];
  ...
  private simulateRealTimeUpdates() {
    setInterval(() => {
      ...
      this.rouletteTables.forEach(table => {
        if (table.gamePhase === 'betting') {
          ...
          if (table.bettingTimeLeft === 0) {
            ...
            setTimeout(() => {
              const number = Math.floor(Math.random() * (table.type === 'american' ? 38 : 37));
        ...
        if (Math.random() > 0.85) {
          ...
          if (table.gamePhase === 'come-out') {
            ...
            if (total === table.point) {
              ...
              table.point = undefined;
              // Change shooter
              const currentShooterIndex = table.players.findIndex(p => p.isShooter);
              if (currentShooterIndex !== -1) {
                table.players[currentShooterIndex].isShooter = false;
                const nextShooterIndex = (currentShooterIndex + 1) % table.players.length;
      ...
      [this.blackjackTables, this.rouletteTables, this.baccaratTables, this.crapsTables].forEach(tableMap => {
  ...
  private simulateRandomChatMessage() {
    ...
    const randomTableId = allTables[Math.floor(Math.random() * allTables.length)];
  ...
  // Blackjack methods
  public hitCard(tableId: string, playerId: string, handIndex: number = 0): void {
  ...
  public doubleDown(tableId: string, playerId: string, handIndex: number = 0): void {
    if (this.socket && this.socket.connected) {
      this.socket.emit('blackjack_double', { tableId, playerId, handIndex });
    }
  }

  public splitHand(tableId: string, playerId: string, handIndex: number = 0): void {
    if (this.socket && this.socket.connected) {
