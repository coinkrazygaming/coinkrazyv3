...
class TickerService {
  ...
  private generateInitialMessages() {
    ...
    this.addMessage({
      type: "win",
      content:
        "🎉 Player **LuckySpinner** just won **25.50 SC** on Sweet Bonanza!",
      priority: "high",
      source: "LuckyAI",
    });
    ...
    this.addMessage({
      type: "new_user",
      content:
        "👋 Welcome **CasinoFan2024** to CoinKrazy! Enjoy your welcome bonus!",
      priority: "medium",
      source: "JoseyAI",
    });
    ...
    this.addMessage({
      type: "leaderboard",
      content:
        "🏆 **GoldRush23** climbs to #1 on the Weekly Leaderboard with 1,247 SC won!",
      priority: "medium",
      source: "LuckyAI",
    });
    ...
    this.addMessage({
      type: "offer",
      content:
        "💰 **Flash Sale**: 50% extra Gold Coins on all packages! Limited time offer!",
      priority: "high",
      source: "LuckyAI",
    });
    ...
    this.addMessage({
      type: "social",
      content:
        "📱 Follow us on Twitter @CoinKrazy for exclusive bonus codes and updates!",
      priority: "low",
      source: "JoseyAI",
    });
    ...
    this.addMessage({
      type: "ai_status",
      content:
        "🤖 **JoseyAI**: All systems operational. Monitoring 247 active players.",
      priority: "low",
      source: "JoseyAI",
    });
  }
  ...
  private generateWinMessage() {
    ...
    const amounts = [5.25, 12.5, 18.75, 25.0, 33.5, 47.25, 55.8, 72.4, 89.9];
  ...
  private generateSocialMessage() {
    ...
    const message =
      socialMessages[Math.floor(Math.random() * socialMessages.length)];
  ...
  // Public methods
  addMessage(
    messageData: Omit<TickerMessage, "id" | "timestamp" | "isActive">,
  ): string {
  ...
  subscribeToTickers(
    callback: (messages: TickerMessage[]) => void,
  ): () => void {
  ...
  postAIMessage(
    ...
    content: string,
    priority: TickerMessage["priority"] = "medium",
  ): string {
  ...
  getTickerStats(): {
    ...
    const averageMessageLifetime =
      messages.length > 0 ? totalLifetime / messages.length / 1000 : 0;
