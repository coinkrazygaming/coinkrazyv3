...
export class SCWinTracker {
  ...
  async trackWin(
    ...
    try {
      ...
      if (amount >= 0.01) {
        ...
        // Log for debugging
        console.log(
          `[SC WIN TRACKER] ${amount.toFixed(2)} SC won by ${userId} in ${gameType}`,
        );
        ...
        console.log(
          `[SC WIN TRACKER] Win amount ${amount} SC below minimum threshold (0.01 SC)`,
        );
        return false;
      }
    } catch (error) {
  ...
  async trackMultipleWins(
    userId: string,
    wins: Array<{ amount: number; gameType: string; gameId?: string }>,
  ) {
    const results = await Promise.allSettled(
      wins.map((win) =>
        this.trackWin(userId, win.amount, win.gameType, win.gameId),
      ),
    );
    ...
    console.log(
      `[SC WIN TRACKER] Tracked ${successful}/${wins.length} wins, total: ${total.toFixed(2)} SC`,
    );
