...
class BankingService {
  ...
  async createPaymentProvider(
    ...
    const values = [
      ...
      provider.api_key_encrypted
        ? this.encrypt(provider.api_key_encrypted)
        : null,
      provider.api_secret_encrypted
        ? this.encrypt(provider.api_secret_encrypted)
        : null,
  ...
  async createTransaction(
    transaction: Partial<BankingTransaction>,
    ...
    const values = [
      ...
      transaction.payment_details
        ? JSON.stringify(transaction.payment_details)
        : null,
      transaction.billing_address
        ? JSON.stringify(transaction.billing_address)
        : null,
  ...
  async processDeposit(
    ...
    try {
      ...
      // Calculate fees
      const feeAmount =
        (amount * paymentMethod.deposit_fee_percent) / 100 +
        paymentMethod.deposit_fee_fixed;
      ...
      if (provider.type === "stripe") {
        providerResult = await this.processStripeDeposit(
          transaction,
          paymentDetails,
        );
      } else if (provider.type === "paypal") {
        providerResult = await this.processPayPalDeposit(
          transaction,
          paymentDetails,
        );
      } else if (provider.type === "crypto") {
        providerResult = await this.processCryptoDeposit(
          transaction,
          paymentDetails,
        );
      } else {
  ...
  private async processStripeDeposit(
  ...
  private async processPayPalDeposit(
    transaction: BankingTransaction,
    paymentDetails: any,
  ) {
  ...
  private async processCryptoDeposit(
    transaction: BankingTransaction,
    paymentDetails: any,
  ) {
  ...
  async getBankingSettings(): Promise<BankingSettings> {
    ...
    return {
      auto_approval: settings.auto_approval || {
        deposits: { enabled: false, max_amount: 0 },
        withdrawals: { enabled: false, max_amount: 0 },
      },
      kyc_requirements: settings.kyc_requirements || {
        level: "basic",
        require_documents: false,
        deposit_limit_no_kyc: 100,
      },
      fraud_detection: settings.fraud_detection || {
        enabled: true,
        risk_threshold: 75,
      ...
      notifications: settings.notifications || {
        large_transactions: { enabled: true, threshold: 5000 },
