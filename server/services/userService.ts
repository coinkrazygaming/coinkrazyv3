...
export interface Transaction {
  id: number;
  transaction_type: 'deposit' | 'withdrawal' | 'win' | 'bet' | 'bonus' | 'transfer';
  currency: 'GC' | 'SC';
  amount: number;
...
class UserService {
  ...
  async updateUserBalance(
    ...
    try {
      ...
      const transactionResult = await client.query(`
        ...
        RETURNING id
      `, [userId, transactionType, currency, amount, currentBalance, newBalance, description, gameId, sessionId]);
  ...
  async getUserTransactions(userId: number, limit: number = 50, offset: number = 0): Promise<Transaction[]> {
    const client = await pool.connect();
